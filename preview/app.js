...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  function mobileDragHandle(button,item,selector,onDrop,label='Move item',lift=item,targetResolver=null){
  ...
  function renderNotebookRow(song,section,line,index){
    ...
    const grip=document.createElement('button');grip.className='line-grip';grip.textContent='⋮⋮';grip.draggable=!phoneShapeLayout();grip.title='Move row';grip.setAttribute('aria-label','Move row');grip.ondragstart=event=>{dragData={kind:'line',lineId:line.id,sectionId:section.id};event.dataTransfer.setData('text/plain',line.id);event.dataTransfer.effectAllowed='move';};grip.ondragend=()=>{dragData=null;row.classList.remove('drop-before','drop-after');};mobileDragHandle(grip,row,'.notebook-row,.write-section',(target,drag)=>{const targetLineId=target.dataset.lineId;if(targetLineId){const found=findLine(song,targetLineId);if(found){const targetLineIndex=found.section.lines.indexOf(found.line);moveLine(song,section.id,line.id,found.section.id,targetLineIndex+(found.section.id===section.id?0:(drag.dropAfter?1:0)));}}else{const targetSection=findSection(song,target.dataset.sectionId);if(targetSection)moveLine(song,section.id,line.id,targetSection.id,targetSection.lines.length);}},'Move row',row,(under,event)=>writeMobileDropTarget(under,event,row,section.id));
  ...
  function nearestWriteChordDrop(row,clientX){const slots=[...row.querySelectorAll('.chord-slot')];if(!slots.length)return null;const slot=slots.reduce((closest,candidate)=>{const bounds=candidate.getBoundingClientRect(),distance=Math.abs(clientX-(bounds.left+bounds.width/2));return !closest||distance<closest.distance?{slot:candidate,distance}:closest;},null)?.slot;if(!slot)return null;return{slot,anchor:Number(slot.dataset.anchor)||0,text:slot.closest('.word-stack')?.querySelector('.word-text')?.textContent?.trim()||''};}
