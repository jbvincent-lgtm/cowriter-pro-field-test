...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  function addMobileShapeSection(song){const section=createSection('New section',[],160,120+song.sections.length*250);section.shape.h=180;song.sections.push(section);touch(song);render();focusMobileShapeEditor(`[data-mobile-section-id="${CSS.escape(section.id)}"] .shape-mobile-section-title`,true);}
  function addMobileShapeLine(song,section,kind='lyric'){const line=createLine('',kind,sectionKey(song,section));section.lines.push(line);section.collapsed=false;mobileResetLinePositions(section);touch(song);render();focusMobileShapeEditor(`[data-mobile-line-id="${CSS.escape(line.id)}"] .shape-mobile-copy`);}
  ...
  function mobileSectionMenu(song,section,anchor){openMenu(anchor,[['Add chord row',()=>addMobileShapeLine(song,section,'progression')],['Delete section',()=>perform('Deleted section',()=>song.sections=song.sections.filter(item=>item.id!==section.id),'Section deleted'),'danger']]);}
  ...
  function renderMobileShape(song){
    ...
    orderedBlocks.forEach((block,index)=>{const card=document.createElement('article');card.className='shape-mobile-fragment';card.dataset.mobileBlockId=block.id;const edit=mobileShapeTextEditor(block.text,'Write a fragment…',block.type==='harmony'?'Progression':'Fragment',value=>{block.text=value;touch(song);},value=>{if(block.type==='fragment'&&looksLikeProgression(value))block.type='harmony';touch(song);});const actions=document.createElement('div');actions.className='shape-mobile-actions';const up=document.createElement('button');up.className='icon-button';up.textContent='↑';up.title='Move up';up.disabled=index===0;up.onclick=()=>{const previous=orderedBlocks[index-1];if(!previous)return;perform('Moved loose line',()=>{const y=block.y;block.y=previous.y;previous.y=y;},'Loose line moved');};const down=document.createElement('button');down.className='icon-button';down.textContent='↓';down.title='Move down';down.disabled=index===orderedBlocks.length-1;down.onclick=()=>{const next=orderedBlocks[index+1];if(!next)return;perform('Moved loose line',()=>{const y=block.y;block.y=next.y;next.y=y;},'Loose line moved');};const move=document.createElement('button');move.className='ghost-button';move.textContent='Move to…';move.onclick=()=>moveMobileBlock(song,block);const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.onclick=()=>openMenu(more,[['Duplicate',()=>duplicateMobileShapeBlock(song,block)],['Delete',()=>perform('Deleted fragment',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Fragment deleted'),'danger']]);actions.append(up,down,move,more);card.append(edit,actions);loose.append(card);});view.append(loose);
