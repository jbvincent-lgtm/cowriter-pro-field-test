...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  function undo() { const item = undoStack.pop(); if (!item) return; redoStack.push({label:item.label,runtime:runtimeSnapshot()}); restoreRuntime(item.runtime); resetActiveSelection(); scheduleSave(); render(); updateUndoButtons(); }
  function redo() { const item = redoStack.pop(); if (!item) return; undoStack.push({label:item.label,runtime:runtimeSnapshot()}); restoreRuntime(item.runtime); resetActiveSelection(); scheduleSave(); render(); updateUndoButtons(); }
