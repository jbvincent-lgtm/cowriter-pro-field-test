(() => {
  ...
  const DEMO_SESSION_KEY = `cowriter-pro-demo-0.4.0.10${RELEASE_CHANNEL === 'stable' ? '' : `-${RELEASE_CHANNEL}`}`;
...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  function phoneToolbarMore(song,anchor,context){
    const items=[];
    if(context==='plan')items.push(['Text appearance',()=>openTextMenu(song,anchor,'plan')],['Player setup',()=>openInlineHarmonyPopover(song,anchor)]);
  ...
  function renderPhoneToolbar(song,left,right){
    if(state.view==='plan')left.append(toolButton('Send to Shape','Copy selected writing to Shape',()=>sendPlanToShape(song),'primary-action'));
    ...
    const more=toolButton('More','Open contextual tools',event=>phoneToolbarMore(song,event.currentTarget,state.view));right.append(more);
  }
  ...
  function mobileTouchPointerMove(event){
    const drag=mobileTouchDrag;if(!drag||event.pointerId!==drag.pointerId)return;event.preventDefault();drag.lastY=event.clientY;const delta=event.clientY-drag.startY;if(Math.abs(delta)>4)drag.moved=true;drag.lift.style.transform=`translate3d(0,${delta}px,0)`;const previousTarget=drag.target;drag.target?.classList.remove('mobile-drop-target');const under=document.elementFromPoint(event.clientX,event.clientY);const target=under?.closest?.(drag.selector);drag.target=target&&target!==drag.item?target:null;drag.target?.classList.add('mobile-drop-target');if(drag.target&&drag.target!==previousTarget)navigator.vibrate?.(4);const host=els.viewHost,rect=host?.getBoundingClientRect();if(rect){const edge=Math.min(82,rect.height*.16);if(event.clientY<rect.top+edge)host.scrollTop-=Math.max(8,(rect.top+edge-event.clientY)*.22);else if(event.clientY>rect.bottom-edge)host.scrollTop+=Math.max(8,(event.clientY-(rect.bottom-edge))*.22);}
  }
  ...
  function renderShapeBlock(song,block) {
    ...
    const head=document.createElement('header');head.className='shape-block-head';const grip=document.createElement('button');grip.className='shape-grip';grip.textContent='⋮⋮';grip.title='Move loose line';grip.onpointerdown=event=>beginShapeObjectPointer(event,song,key,true);const label=document.createElement('span');label.className='shape-label';label.textContent=block.type==='harmony'?'Progression':block.type==='title'?'Title':'';const more=document.createElement('button');more.className='shape-more';more.textContent='•••';more.onclick=event=>{event.stopPropagation();openShapeBlockMenu(song,block,more);};head.append(grip,label,more);article.append(head);
  ...
  function openShapeBlockMenu(song,block,anchor){
    ...
    openMenu(anchor,[['Edit loose line',()=>{shapeEditingKey=shapeKey('block',block.id);shapeSelection.clear();shapeSelection.add(shapeEditingKey);rememberShapeSelection(song);renderView(song);}],['Place in song',()=>{shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));sendShapeSelectionToWrite(song);}],...(destinations.length?[['Place in section…',()=>openChoiceModal('Place in section',destinations)]]:[]),['Convert to loose line',()=>{block.type='fragment';touch(song);render();}],['Convert to title',()=>{block.type='title';touch(song);render();}],['Convert to progression',()=>{block.type='harmony';touch(song);render();}],['Delete',()=>perform('Deleted loose line',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Loose line deleted'),'danger']]);
  }
  ...
  function alternativeToSpare(song,alt){perform('Saved alternative as loose line',()=>{if(alt.content?.line){song.shapeBlocks.push(createBlock(alt.content.line.kind==='progression'?'harmony':'fragment',alt.content.line.text,180,140,alt.content.line.chords));}else song.shapeBlocks.push(createBlock(alt.type==='harmony'?'harmony':alt.type==='title'?'title':'fragment',alternativePreview(alt),180,140));song.alternatives=song.alternatives.filter(item=>item.id!==alt.id);},'Saved to Loose lines');}
  ...
  function renderWorkbenchSpare(song){
    ...
    song.shapeBlocks.forEach(block=>{
      ...
      card.innerHTML=`${block.type==='harmony'||block.type==='title'?`<div class="shelf-meta">${escapeHtml(block.type==='harmony'?'Progression':'Title')}</div>`:''}<div class="shelf-copy">${escapeHtml(block.text||'Blank loose line')}</div>`;
  ...
  function openChartExport(song){openChoiceModal('Export chart',[['Print / PDF',()=>window.print()],['Plain text',()=>downloadText(`${safeName(song.title)}.txt`,chartText(song))],['ChordPro',()=>downloadText(`${safeName(song.title)}.cho`,chordProText(song))],['Complete song bundle',()=>exportSongBundle(song)],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.10',song},null,2))]]);}
  ...
  function openSongMenu(anchor){
    const song=currentSong();if(!song)return;
    if(inSandboxMode()){openMenu(anchor,[['Return to demo song',returnToDemoSong],['Reset this sandbox song',()=>{if(confirm(`Reset “${song.title}” to its seed data?`))resetCurrentSandboxSong();}],['Reset sandbox library',()=>{if(confirm('Reset all eight sandbox songs?'))resetSandboxLibrary();}],['Save a copy',saveCurrentSandboxCopy],['Exit demo',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],['Export sandbox song JSON',()=>downloadText(`${safeName(song.title)}-sandbox.json`,JSON.stringify({type:'cowriter-song-0.4.0.10',song},null,2))]]);return;}
    ...
    openMenu(anchor,[['Start review capture',startReviewCapture],['Export complete song…',()=>exportSongBundle(song)],['Studio Log',()=>openStudioLog(song)],['Record Work Tape',()=>startWorkTape(song)],['Change progress…',()=>openStagePicker(song,anchor)],['Add to project…',()=>openProjectMembership(song)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Key, capo and tuning…',()=>openKeyCapoPanel(song)],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],[song.favourite?'Remove favourite':'Favourite',()=>{song.favourite=!song.favourite;touch(song);render();}],['Duplicate song',()=>duplicateSong(song)],['Export song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.10',song},null,2))],['Archive',()=>{song.libraryStatus='archived';touch(song);render();}],['Move to Trash',()=>{snapshot('Move song to Trash');state.deletedSongs.unshift(song);state.songs=state.songs.filter(item=>item.id!==song.id);state.selectedSongId=state.songs[0]?.id||null;scheduleSave();render();toast('Song moved to Trash',undo);},'danger']]);
  }
  ...
  async function exportSongBundle(song){
    ...
    const manifest={type:'cowriter-song-bundle',version:'0.4.0.10',exchangeId,createdAt,contributor,sourceSongId:song.sourceSongId||song.id,songId:song.id,title:song.title,audio,missingAudio:missing};
    files['manifest.json']=JSON.stringify(manifest,null,2);files['song.json']=JSON.stringify(song,null,2);files['README.txt']=`Co-Writer 0.4.0.10 Field Exchange\n\nSong: ${song.title}\nSent by: ${contributor}\nCreated: ${new Date(createdAt).toLocaleString()}\n\nImport this .cowriter-song.zip from Welcome or Library.\n${missing.length?`\nMissing audio: ${missing.join(', ')}\n`:''}`;
  ...
  function exportBackup(){downloadText(`Co-Writer-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({type:'cowriter-pro-backup',version:'0.4.0.10',exportedAt:now(),audioNotice:'Use Export complete song for a transferable bundle containing Work Tapes.',state},null,2));}
  ...
  function bind(){
    ...
    els.libraryMenu.onclick=()=>openMenu(els.libraryMenu,[['Explore demo',loadDemo],['Start review capture',startReviewCapture],['Import song or backup…',chooseImport],['Backup library metadata',exportBackup],['Install for offline',installForOffline],['About',openAbout]]);
