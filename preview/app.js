(() => {
  ...
  const DEMO_SESSION_KEY = `cowriter-pro-demo-0.4.0.9${RELEASE_CHANNEL === 'stable' ? '' : `-${RELEASE_CHANNEL}`}`;
...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  const DEMO_STEPS = [
    ...
    {view:'shape',mode:'Shape',title:'Move the pieces',copy:'Shape arranges sections and loose lines. Pieces can stay unplaced while you look for the form.',task:'Drag “The porch light stayed on…” to a place that feels right.',target:'[data-demo-key="porch-light"]'},
  ...
  function openIdeas(){if(inDemoMode())exitDemo(false);state.shellPage='ideas';state.sidebarCollapsed=true;scheduleSave();render();}
  ...
  async function useIdeaInSong(idea,song,openAfter=true){
    const text=idea.text.trim();if(text)song.plan.brainDump=[song.plan.brainDump.trim(),text].filter(Boolean).join('\n\n');
    await copyIdeaTakesToSong(idea,song);if(!idea.usedInSongIds.includes(song.id))idea.usedInSongIds.push(song.id);idea.updatedAt=now();touch(song);
    if(openAfter){state.selectedSongId=song.id;state.view='plan';state.shellPage='workspace';state.sidebarCollapsed=true;render();toast(`Added to ${song.title}`);}
  }
  ...
  function chooseSongForIdea(idea){
    ...
    openChoiceModal('Use in a song',state.songs.slice().sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt)).map(song=>[song.title,()=>useIdeaInSong(idea,song)]),'The original will stay in Ideas.');
  }
  ...
  async function deleteIdea(idea){
    ...
    state.ideas=state.ideas.filter(item=>item.id!==idea.id);scheduleSave();renderIdeas();toast('Idea deleted');
  }
  ...
  function renderIdeas(){
    ...
    ideas.forEach(idea=>{const card=document.createElement('article');card.className='idea-card';
      const copy=document.createElement('button');copy.className='idea-card-copy';copy.onclick=()=>openIdeaComposer(idea);const text=idea.text.trim();copy.innerHTML=`<strong>${escapeHtml(ideaTitle(idea))}</strong>${text?`<p>${escapeHtml(text.slice(0,320))}</p>`:'<p class="idea-audio-label">Recorded idea</p>'}<small>${escapeHtml(relativeDate(idea.updatedAt))}${idea.usedInSongIds.length?' · used in a song':''}</small>`;
      ...
      const actions=document.createElement('div');actions.className='idea-card-actions';const edit=document.createElement('button');edit.className='text-button';edit.textContent='Edit';edit.onclick=()=>openIdeaComposer(idea);const turn=document.createElement('button');turn.className='primary-button';turn.textContent='Turn into Song';turn.onclick=()=>turnIdeaIntoSong(idea);const use=document.createElement('button');use.className='ghost-button';use.textContent='Use in Song';use.onclick=()=>chooseSongForIdea(idea);const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.title='Idea menu';more.onclick=()=>openMenu(more,[[idea.favourite?'Remove favourite':'Favourite',()=>{idea.favourite=!idea.favourite;idea.updatedAt=now();scheduleSave();renderIdeas();}],['Delete idea',()=>deleteIdea(idea),'danger']]);actions.append(edit,use,turn,more);card.append(copy);if(idea.takeIds.length)card.append(audio);card.append(actions);els.ideaList.append(card);});
  }
  ...
  function render() {
    ...
    els.songContext.innerHTML = `${inSandboxMode()?'<span class="demo-badge sandbox-badge">SANDBOX · NOT IN LIBRARY</span>':inDemoMode()?'<span class="demo-badge">DEMO · NOT IN LIBRARY</span>':''}`;
    renderToolbar(song);
    renderView(song);
    renderWorkbench();
  ...
  function scrollWorkspaceToTop(){els.viewHost.scrollTo({top:0,behavior:window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});}
  ...
  function renderToolbar(song) {
    ...
    if (state.view==='plan') {
      left.append(toolButton('Send to Shape','Copy the selection or current paragraph to Shape',()=>sendPlanToShape(song),'primary-action'));right.append(toolButton(playerSetupLabel(song),'Sounding key, playing shapes and capo',event=>openInlineHarmonyPopover(song,event.currentTarget),'music-setup'),toolButton('Text','Point size and restrained formatting',event=>openTextMenu(song,event.currentTarget,'plan')),toolButton('Words & Music','Open words or music',()=>toggleWorkbench(state.workbenchTab||'words'),state.workbenchOpen?'active':''));
    } else if (state.view==='shape') {
      if(phoneShapeLayout()){
        left.append(toolButton('+ Fragment','Add a loose fragment',()=>addMobileShapeBlock(song),'primary-action'),toolButton('+ Section','Add a section',()=>addMobileShapeSection(song)));
  ...
  function phoneShapeLayout(){return window.matchMedia?.('(max-width: 700px)').matches;}
  ...
  function editMobileShapeBlock(song,block){openFormModal('Edit fragment',[{name:'text',label:block.type==='harmony'?'Progression':'Text',type:'textarea',value:block.text}],values=>{block.text=values.text;touch(song);render();});}
  function addMobileShapeBlock(song){openFormModal('New fragment',[{name:'text',label:'Text',type:'textarea',placeholder:'A line, title, chord, or thought…',required:true}],values=>{const block=createBlock(looksLikeProgression(values.text)?'harmony':'fragment',values.text,160,100+song.shapeBlocks.length*120);song.shapeBlocks.push(block);touch(song);render();});}
  function addMobileShapeSection(song){openFormModal('New section',[{name:'label',label:'Section name',type:'text',placeholder:'Verse 1',required:true}],values=>{const section=createSection(values.label.trim(),[],160,120+song.sections.length*250);section.shape.h=180;song.sections.push(section);touch(song);render();});}
  function duplicateMobileShapeBlock(song,block){perform('Duplicated fragment',()=>{const copy=clone(block);copy.id=uid('block');copy.x+=24;copy.y+=70;song.shapeBlocks.push(copy);},'Fragment duplicated');}
  function moveMobileBlock(song,block){const choices=song.sections.map(section=>[section.label,()=>addShapeBlockToSection(song,block,section)]);if(!choices.length){toast('Add a section first');return;}openChoiceModal('Move fragment to',choices,'It becomes a line in that section.');}
  function editMobileShapeLine(song,section,line){openFormModal(line.kind==='progression'?'Edit chord row':'Edit line',[{name:'text',label:line.kind==='progression'?'Progression':'Text',type:'textarea',value:line.text}],values=>{line.text=values.text;if(line.kind==='lyric')line.chords=NE.normaliseChords(line.chords,line.text.length);mobileResetLinePositions(section);touch(song);render();});}
  ...
  function mobileSectionMenu(song,section,anchor){openMenu(anchor,[['Rename…',()=>openFormModal('Rename section',[{name:'label',label:'Section name',type:'text',value:section.label,required:true}],values=>{section.label=values.label.trim();touch(song);render();})],['Add lyric line',()=>{const line=createLine('', 'lyric',sectionKey(song,section));section.lines.push(line);mobileResetLinePositions(section);touch(song);render();requestAnimationFrame(()=>editMobileShapeLine(song,section,line));}],['Add chord row',()=>{const line=createLine('', 'progression',sectionKey(song,section));section.lines.push(line);mobileResetLinePositions(section);touch(song);render();requestAnimationFrame(()=>editMobileShapeLine(song,section,line));}],['Delete section',()=>perform('Deleted section',()=>song.sections=song.sections.filter(item=>item.id!==section.id),'Section deleted'),'danger']]);}

  function renderMobileShape(song){
    const view=document.createElement('div');view.className='shape-mobile';const intro=document.createElement('header');intro.className='shape-mobile-intro';intro.innerHTML='<div><span>SHAPE</span><h2>Arrange the song</h2></div><p>Move loose lines into sections. Reorder them until the form feels right.</p>';if(song.sections.length){const jump=document.createElement('button');jump.className='ghost-button shape-mobile-jump';jump.textContent='Jump to section';jump.onclick=()=>openChoicePopover(jump,'Go to section',[['Loose lines',()=>document.querySelector('.shape-mobile-group.loose')?.scrollIntoView({behavior:'smooth',block:'start'})],...song.sections.map(section=>[section.label,()=>{const target=document.querySelector(`[data-mobile-section-id="${CSS.escape(section.id)}"]`);if(target){target.open=true;target.scrollIntoView({behavior:'smooth',block:'start'});}}])]);intro.append(jump);}view.append(intro);
    ...
    orderedBlocks.forEach((block,index)=>{const card=document.createElement('article');card.className='shape-mobile-fragment';const edit=document.createElement('button');edit.className='shape-mobile-copy';edit.textContent=block.text||'Blank fragment';edit.onclick=()=>editMobileShapeBlock(song,block);const actions=document.createElement('div');actions.className='shape-mobile-actions';const up=document.createElement('button');up.className='icon-button';up.textContent='↑';up.title='Move up';up.disabled=index===0;up.onclick=()=>{const previous=orderedBlocks[index-1];if(!previous)return;perform('Moved loose line',()=>{const y=block.y;block.y=previous.y;previous.y=y;},'Loose line moved');};const down=document.createElement('button');down.className='icon-button';down.textContent='↓';down.title='Move down';down.disabled=index===orderedBlocks.length-1;down.onclick=()=>{const next=orderedBlocks[index+1];if(!next)return;perform('Moved loose line',()=>{const y=block.y;block.y=next.y;next.y=y;},'Loose line moved');};const move=document.createElement('button');move.className='ghost-button';move.textContent='Move to…';move.onclick=()=>moveMobileBlock(song,block);const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.onclick=()=>openMenu(more,[['Edit',()=>editMobileShapeBlock(song,block)],['Duplicate',()=>duplicateMobileShapeBlock(song,block)],['Delete',()=>perform('Deleted fragment',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Fragment deleted'),'danger']]);actions.append(up,down,move,more);card.append(edit,actions);loose.append(card);});view.append(loose);
    ...
    song.sections.forEach((section,sectionIndex)=>{const card=document.createElement('details');card.className='shape-mobile-section';card.dataset.mobileSectionId=section.id;card.open=!section.collapsed;card.ontoggle=()=>{section.collapsed=!card.open;scheduleSave();};const summary=document.createElement('summary');summary.innerHTML=`<strong>${escapeHtml(section.label)}</strong><small>${section.lines.length} ${section.lines.length===1?'line':'lines'}</small>`;const controls=document.createElement('div');controls.className='shape-mobile-actions shape-mobile-section-actions';const up=document.createElement('button');up.className='icon-button';up.textContent='↑';up.title='Move section up';up.disabled=sectionIndex===0;up.onclick=()=>mobileMoveSection(song,section,-1);const down=document.createElement('button');down.className='icon-button';down.textContent='↓';down.title='Move section down';down.disabled=sectionIndex===song.sections.length-1;down.onclick=()=>mobileMoveSection(song,section,1);const write=document.createElement('button');write.className='ghost-button';write.textContent='Write';write.onclick=()=>{activeSectionId=section.id;activeLineId=section.lines[0]?.id||null;setView('write');requestAnimationFrame(()=>document.getElementById(`write-${section.id}`)?.scrollIntoView({block:'start'}));};const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.onclick=()=>mobileSectionMenu(song,section,more);controls.append(up,down,write,more);card.append(summary,controls);
      const list=document.createElement('div');list.className='shape-mobile-lines';if(!section.lines.length){const empty=document.createElement('p');empty.className='shape-mobile-empty';empty.textContent='No lines in this section.';list.append(empty);}section.lines.forEach((line,lineIndex)=>{const row=document.createElement('article');row.className='shape-mobile-line';const copy=document.createElement('button');copy.className='shape-mobile-copy';copy.textContent=line.text||(line.kind==='progression'?'Blank chord row':'Blank line');copy.onclick=()=>editMobileShapeLine(song,section,line);const rowActions=document.createElement('div');rowActions.className='shape-mobile-actions';const lineUp=document.createElement('button');lineUp.className='icon-button';lineUp.textContent='↑';lineUp.title='Move line up';lineUp.disabled=lineIndex===0;lineUp.onclick=()=>mobileMoveLine(song,section,line,section,lineIndex-1);const lineDown=document.createElement('button');lineDown.className='icon-button';lineDown.textContent='↓';lineDown.title='Move line down';lineDown.disabled=lineIndex===section.lines.length-1;lineDown.onclick=()=>mobileMoveLine(song,section,line,section,lineIndex+1);const move=document.createElement('button');move.className='text-button';move.textContent='Move';move.onclick=()=>moveMobileShapeLine(song,section,line);rowActions.append(lineUp,lineDown,move);row.append(copy,rowActions);list.append(row);});const add=document.createElement('button');add.className='shape-mobile-add-line';add.textContent='+ Add line';add.onclick=()=>{const line=createLine('', 'lyric',sectionKey(song,section));section.lines.push(line);mobileResetLinePositions(section);touch(song);render();requestAnimationFrame(()=>editMobileShapeLine(song,section,line));};list.append(add);card.append(list);sections.append(card);});view.append(sections);els.viewHost.append(view);
  }
  ...
  function renderShapeSectionLine(song,section,line){
    ...
    else{const copy=document.createElement('div');copy.className='shape-line-copy';copy.textContent=line.kind==='progression'?displayProgression(line.text,song,state.chordDisplay,sectionKey(song,section)):(line.text||'Blank line');chip.append(copy);const more=document.createElement('button');more.className='shape-line-more';more.textContent='•••';more.title='Line options';more.onclick=event=>{event.stopPropagation();openMenu(more,[['Make loose',()=>liftLineToShape(song,section,line,section.shape.x+line.shape.x,section.shape.y+SHAPE_SECTION_HEADER+line.shape.y)],['Open in Write',()=>{activeSectionId=section.id;activeLineId=line.id;setView('write');requestAnimationFrame(()=>document.querySelector(`[data-line-id="${CSS.escape(line.id)}"]`)?.scrollIntoView({block:'center'}));}],['Delete',()=>perform('Deleted line',()=>section.lines=section.lines.filter(item=>item.id!==line.id),'Line deleted'),'danger']]);};chip.append(more);}
  ...
  function addShapeBlock(song,type='fragment',point=null){const center=point||visibleShapeCenter(song);snapshot('Created Shape fragment');const block=createBlock(type,'',center.x,center.y);const open=findOpenShapePoint(song,block.w,block.h,center);block.x=open.x;block.y=open.y;song.shapeBlocks.push(block);shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));rememberShapeSelection(song);touch(song);shapeEditingKey=shapeKey('block',block.id);render();}
  ...
  function renderChordToken(song,section,line,chord){const button=document.createElement('button');button.className=`chord-token${selectedChordIds.has(chord.id)||chord.id===activeChordId?' selected':''}${chord.needsReview?' needs-review':''}`;button.title=chord.needsReview?'Check chord position':'Select chord';button.draggable=true;button.setAttribute('aria-label',`Chord ${storedChordToDisplay(chord.value,song,state.chordDisplay,sectionKey(song,section))}`);const main=document.createElement('span');main.textContent=storedChordToDisplay(chord.value,song,state.chordDisplay,sectionKey(song,section));button.append(main);const profile=activeProfile(song);if(profile.showSounding&&state.chordDisplay==='shapes'){const secondary=document.createElement('small');secondary.textContent=chord.value;button.append(secondary);}if(profile.showNashville&&state.chordDisplay!=='numbers'){const number=document.createElement('small');number.textContent=CE.chordToNashville(chord.value,sectionKey(song,section));button.append(number);}button.onpointerdown=event=>event.stopPropagation();button.ondragstart=event=>{event.stopPropagation();dragData={kind:'chord',lineId:line.id,chordId:chord.id};event.dataTransfer.setData('text/plain',chord.id);event.dataTransfer.effectAllowed='move';};button.ondragend=()=>dragData=null;button.onclick=event=>{event.preventDefault();event.stopPropagation();if(activeChordLineId!==line.id)selectedChordIds.clear();if(event.shiftKey&&activeChordId){const ordered=(line.chords||[]).slice().sort((a,b)=>a.anchor-b.anchor),from=ordered.findIndex(item=>item.id===activeChordId),to=ordered.findIndex(item=>item.id===chord.id);if(from>=0&&to>=0)ordered.slice(Math.min(from,to),Math.max(from,to)+1).forEach(item=>selectedChordIds.add(item.id));}else if(event.metaKey||event.ctrlKey){if(selectedChordIds.has(chord.id))selectedChordIds.delete(chord.id);else selectedChordIds.add(chord.id);}else{selectedChordIds.clear();selectedChordIds.add(chord.id);}activeLineId=line.id;activeSectionId=section.id;activeChordId=chord.id;activeChordLineId=line.id;activeProgressionToken=null;pendingChordAnchor=null;toggleWorkbench('music');renderView(song);};return button;}

  function openLineEditor(song,section,line,cursor=null){activeLineId=line.id;activeSectionId=section.id;activeChordId=null;activeChordLineId=null;selectedChordIds.clear();activeProgressionToken=null;pendingChordAnchor=null;const existing=document.querySelector(`.notebook-row[data-line-id="${CSS.escape(line.id)}"] .line-editor`);if(existing){existing.focus();if(cursor!==null)existing.selectionStart=existing.selectionEnd=clamp(cursor,0,existing.value.length);return;}if(activeEditor)activeEditor.blur();const row=document.querySelector(`.notebook-row[data-line-id="${CSS.escape(line.id)}"]`);if(!row)return;row.classList.add('active');const paper=row.querySelector('.line-paper');const display=paper.querySelector('.notebook-content,.chord-row-content');if(!display)return;const textarea=document.createElement('textarea');textarea.className=`line-editor${line.kind==='progression'?' chord-row-editor':''}`;textarea.value=line.kind==='progression'?displayProgression(line.text,song,state.chordDisplay,sectionKey(song,section)):line.text;display.replaceWith(textarea);activeEditor=textarea;textarea.focus();textarea.selectionStart=textarea.selectionEnd=cursor===null?textarea.value.length:clamp(cursor,0,textarea.value.length);autoGrow(textarea,42);
  ...
  function renderWorkbenchSpare(song){
    ...
    song.shapeBlocks.forEach(block=>{
      ...
      const use=document.createElement('button');use.textContent=state.view==='plan'?'Copy to Plan':state.view==='shape'?'Place in Shape':'Place in song';
      use.onclick=()=>{
        if(state.view==='plan'){
  ...
  function openChartExport(song){openChoiceModal('Export chart',[['Print / PDF',()=>window.print()],['Plain text',()=>downloadText(`${safeName(song.title)}.txt`,chartText(song))],['ChordPro',()=>downloadText(`${safeName(song.title)}.cho`,chordProText(song))],['Complete song bundle',()=>exportSongBundle(song)],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.9',song},null,2))]]);}
  ...
  function openSongMenu(anchor){
    const song=currentSong();if(!song)return;
    if(inSandboxMode()){openMenu(anchor,[['Return to demo song',returnToDemoSong],['Reset this sandbox song',()=>{if(confirm(`Reset “${song.title}” to its seed data?`))resetCurrentSandboxSong();}],['Reset sandbox library',()=>{if(confirm('Reset all eight sandbox songs?'))resetSandboxLibrary();}],['Save a copy',saveCurrentSandboxCopy],['Exit demo',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],['Export sandbox song JSON',()=>downloadText(`${safeName(song.title)}-sandbox.json`,JSON.stringify({type:'cowriter-song-0.4.0.9',song},null,2))]]);return;}
    ...
    openMenu(anchor,[['Export complete song…',()=>exportSongBundle(song)],['Studio Log',()=>openStudioLog(song)],['Record Work Tape',()=>startWorkTape(song)],['Change progress…',()=>openStagePicker(song,anchor)],['Add to project…',()=>openProjectMembership(song)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Key, capo and tuning…',()=>openKeyCapoPanel(song)],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],[song.favourite?'Remove favourite':'Favourite',()=>{song.favourite=!song.favourite;touch(song);render();}],['Duplicate song',()=>duplicateSong(song)],['Export song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.9',song},null,2))],['Archive',()=>{song.libraryStatus='archived';touch(song);render();}],['Move to Trash',()=>{snapshot('Move song to Trash');state.deletedSongs.unshift(song);state.songs=state.songs.filter(item=>item.id!==song.id);state.selectedSongId=state.songs[0]?.id||null;scheduleSave();render();toast('Song moved to Trash',undo);},'danger']]);
  }
  ...
  function renderDemoGuide(){
    ...
    const purpose=document.createElement('div');purpose.className='demo-guide-purpose demo-guide-safety';purpose.innerHTML='<strong>Safe demonstration</strong><span>This song is not in your Library. Change anything.</span>';
  ...
  async function exportSongBundle(song){
    ...
    const manifest={type:'cowriter-song-bundle',version:'0.4.0.9',exchangeId,createdAt,contributor,sourceSongId:song.sourceSongId||song.id,songId:song.id,title:song.title,audio,missingAudio:missing};
    files['manifest.json']=JSON.stringify(manifest,null,2);files['song.json']=JSON.stringify(song,null,2);files['README.txt']=`Co-Writer 0.4.0.9 Field Exchange\n\nSong: ${song.title}\nSent by: ${contributor}\nCreated: ${new Date(createdAt).toLocaleString()}\n\nImport this .cowriter-song.zip from Welcome or Library.\n${missing.length?`\nMissing audio: ${missing.join(', ')}\n`:''}`;
  ...
  function exportBackup(){downloadText(`Co-Writer-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({type:'cowriter-pro-backup',version:'0.4.0.9',exportedAt:now(),audioNotice:'Use Export complete song for a transferable bundle containing Work Tapes.',state},null,2));}
  ...
  function bind(){
    ...
    els.welcomeMore.onclick=()=>openChoicePopover(els.welcomeMore,'More ways to begin',[['Explore demo',()=>enterDemo(false)],['Play unattended tour',startUnattendedDemo],['Import song or backup…',chooseImport],['Install for offline',installForOffline],['About',openAbout]]);
    els.emptyOpenLibrary.onclick=()=>{state.shellPage='library';scheduleSave();render();};
    els.ideasHome.onclick=()=>{state.shellPage='welcome';scheduleSave();render();};els.ideasSongs.onclick=openLibrary;
    ...
    els.libraryMenu.onclick=()=>openMenu(els.libraryMenu,[['Explore demo',loadDemo],['Import song or backup…',chooseImport],['Backup library metadata',exportBackup],['Install for offline',installForOffline],['About',openAbout]]);
    ...
    const takeOverTour=()=>{if(!demoTourTimer)return;stopUnattendedDemo();if(demoSession)demoSession.unattended=false;scheduleSave();renderDemoGuide();toast('Tour paused — you’re in control');};document.addEventListener('pointerdown',takeOverTour,true);document.addEventListener('keydown',takeOverTour,true);
    ...
    document.addEventListener('keydown',event=>{const mod=event.metaKey||event.ctrlKey,editing=['INPUT','TEXTAREA'].includes(document.activeElement?.tagName);if(event.code==='Space'&&!editing){spaceHeld=true;if(state.view==='shape')event.preventDefault();}if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='c'&&activeLineId){event.preventDefault();copyActiveChords(currentSong());}else if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='x'&&activeLineId){event.preventDefault();copyActiveChords(currentSong());removeActiveChords(currentSong());}else if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='v'&&activeLineId){event.preventDefault();pasteActiveChords(currentSong());}else if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='d'&&activeLineId){event.preventDefault();duplicateActiveChords(currentSong());}else if(state.view==='write'&&!editing&&(event.key==='Backspace'||event.key==='Delete')&&(activeChordId||activeProgressionToken)){event.preventDefault();removeActiveChords(currentSong());}else if(!editing&&!mod&&event.key==='?'){event.preventDefault();openShortcutsPanel();}else if(mod&&!event.shiftKey&&event.key.toLowerCase()==='z'){event.preventDefault();undo();}else if(mod&&event.shiftKey&&event.key.toLowerCase()==='z'){event.preventDefault();redo();}else if(mod&&event.key.toLowerCase()==='n'){event.preventDefault();createNewSong();}else if(mod&&event.key.toLowerCase()==='f'&&currentSong()){event.preventDefault();openSongSearch(currentSong());}else if(mod&&event.key.toLowerCase()==='k'&&state.view==='write'){event.preventDefault();addChordFromToolbar(currentSong());}else if(state.view==='shape'&&!mod&&!editing&&event.key.toLowerCase()==='v'){setShapeTool('select',currentSong());}else if(state.view==='shape'&&!mod&&!editing&&event.key.toLowerCase()==='h'){setShapeTool('hand',currentSong());}else if(event.key==='Escape'){closeSelectionBar();closeOverlay();if(state.alternativesTrayOpen){closeAlternativesTray();}else if(transientPanel){transientPanel=null;state.workbenchOpen=false;scheduleSave();render();}else if(activeEditor)activeEditor.blur();}});
