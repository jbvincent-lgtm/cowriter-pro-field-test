...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  function mobileDragHandle(button,item,selector,onDrop,label='Move item',lift=item){
    ...
    button.onpointerdown=event=>{if(!phoneShapeLayout())return;event.preventDefault();event.stopPropagation();closeOverlay();mobileTouchDrag={pointerId:event.pointerId,startY:event.clientY,lastY:event.clientY,item,lift,selector,onDrop,target:null,moved:false};button.setPointerCapture?.(event.pointerId);item.classList.add('mobile-drag-source');lift.classList.add('mobile-dragging');document.body.classList.add('mobile-reordering');navigator.vibrate?.(7);};
  }
  function mobileTouchPointerMove(event){
    const drag=mobileTouchDrag;if(!drag||event.pointerId!==drag.pointerId)return;event.preventDefault();drag.lastY=event.clientY;const delta=event.clientY-drag.startY;if(Math.abs(delta)>4)drag.moved=true;drag.lift.style.transform=`translate3d(0,${delta}px,0)`;drag.target?.classList.remove('mobile-drop-target');const under=document.elementFromPoint(event.clientX,event.clientY);const target=under?.closest?.(drag.selector);drag.target=target&&target!==drag.item?target:null;drag.target?.classList.add('mobile-drop-target');const host=els.viewHost,rect=host?.getBoundingClientRect();if(rect){const edge=Math.min(82,rect.height*.16);if(event.clientY<rect.top+edge)host.scrollTop-=Math.max(8,(rect.top+edge-event.clientY)*.22);else if(event.clientY>rect.bottom-edge)host.scrollTop+=Math.max(8,(event.clientY-(rect.bottom-edge))*.22);}
  }
  function mobileTouchPointerUp(event){
    const drag=mobileTouchDrag;if(!drag||event.pointerId!==drag.pointerId)return;mobileTouchDrag=null;drag.target?.classList.remove('mobile-drop-target');drag.item.classList.remove('mobile-drag-source');drag.lift.classList.remove('mobile-dragging');drag.lift.style.transform='';document.body.classList.remove('mobile-reordering');if(event.type!=='pointercancel'&&drag.moved&&drag.target)drag.onDrop(drag.target);
  }
  ...
  function renderMobileShape(song){
    ...
    orderedBlocks.forEach((block,index)=>{
      ...
      const edit=mobileShapeTextEditor(block.text,'Write a fragment…',block.type==='harmony'?'Progression':'Fragment',value=>{block.text=value;touch(song);},value=>{if(block.type==='fragment'&&looksLikeProgression(value))block.type='harmony';touch(song);});
      const actions=document.createElement('div');actions.className='shape-mobile-actions';const drag=document.createElement('button');drag.className='icon-button';drag.textContent='⋮⋮';mobileDragHandle(drag,card,'[data-mobile-block-id]',target=>mobileReorderBlocks(song,block,song.shapeBlocks.find(item=>item.id===target.dataset.mobileBlockId)),'Move fragment');
      ...
      const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.onclick=()=>{const items=[];if(index>0)items.push(['Move up',()=>mobileReorderBlocks(song,block,orderedBlocks[index-1])]);if(index<orderedBlocks.length-1)items.push(['Move down',()=>mobileReorderBlocks(song,block,orderedBlocks[index+1])]);items.push(['Duplicate',()=>duplicateMobileShapeBlock(song,block)],['Delete',()=>perform('Deleted fragment',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Fragment deleted'),'danger']);openMenu(more,items);};
    ...
    song.sections.forEach(section=>{
      ...
      section.lines.forEach((line,lineIndex)=>{
        ...
        const rowActions=document.createElement('div');rowActions.className='shape-mobile-actions';const lineDrag=document.createElement('button');lineDrag.className='icon-button';lineDrag.textContent='⋮⋮';mobileDragHandle(lineDrag,row,'[data-mobile-line-id],.shape-mobile-section',target=>{const targetLineId=target.dataset.mobileLineId;if(targetLineId){const found=findLine(song,targetLineId);if(found)mobileMoveLine(song,section,line,found.section,found.section.lines.indexOf(found.line));}else{const targetSection=findSection(song,target.dataset.mobileSectionId);if(targetSection)mobileMoveLine(song,section,line,targetSection,targetSection.lines.length);}},'Move line');const move=document.createElement('button');move.className='text-button';move.textContent='Move to';move.onclick=()=>moveMobileShapeLine(song,section,line);const more=document.createElement('button');more.className='icon-button';more.textContent='•••';more.onclick=()=>{const items=[];if(lineIndex>0)items.push(['Move up',()=>mobileMoveLine(song,section,line,section,lineIndex-1)]);if(lineIndex<section.lines.length-1)items.push(['Move down',()=>mobileMoveLine(song,section,line,section,lineIndex+1)]);items.push(['Move elsewhere…',()=>moveMobileShapeLine(song,section,line)]);openMenu(more,items);};rowActions.append(lineDrag,move,more);row.append(copy,rowActions);list.append(row);
      });
  ...
  function renderNotebookRow(song,section,line,index){
    ...
    const grip=document.createElement('button');grip.className='line-grip';grip.textContent='⋮⋮';grip.draggable=!phoneShapeLayout();grip.title='Move row';grip.setAttribute('aria-label','Move row');grip.ondragstart=event=>{dragData={kind:'line',lineId:line.id,sectionId:section.id};event.dataTransfer.setData('text/plain',line.id);event.dataTransfer.effectAllowed='move';};grip.ondragend=()=>{dragData=null;row.classList.remove('drop-before','drop-after');};mobileDragHandle(grip,row,'.notebook-row,.write-section',target=>{const targetLineId=target.dataset.lineId;if(targetLineId){const found=findLine(song,targetLineId);if(found)moveLine(song,section.id,line.id,found.section.id,found.section.lines.indexOf(found.line));}else{const targetSection=findSection(song,target.dataset.sectionId);if(targetSection)moveLine(song,section.id,line.id,targetSection.id,targetSection.lines.length);}},'Move row');
