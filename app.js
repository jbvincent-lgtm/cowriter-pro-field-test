(() => {
  ...
  const DB_NAME = 'cowriter-pro-01';
  const DB_VERSION = 2;
  const STORE = 'app';
  const AUDIO_STORE = 'audio';
  const STATE_KEY = 'state';
  const DEMO_SESSION_KEY = 'cowriter-pro-demo-0.4.0.5';
  ...
  const els = Object.fromEntries([
    'app','welcomePage','welcomeContinueCard','welcomeContinueTitle','welcomeContinueMeta','welcomeContinue','welcomeNewSong','welcomeLibrary','welcomeMorningLines','welcomeRecordIdea','welcomeInstall','welcomeConnection','welcomeDemo','welcomeImport','sidebar','libraryHome','libraryMenu','collapseSidebar','expandSidebar','homeButton','mainShell','newSong','emptyNewSong','emptyOpenLibrary','songSearch','songList','libraryFilters','libraryTools','libraryFilterButton','librarySortButton','projectsPanel','projectList','newProject','loadDemo','importBackup','exportBackup','backupFile',
    'saveState','fieldStatus','modeNav','undoButton','redoButton','themeToggle','songMenu','emptyState','workspace','songTitle','songContext','utilityToolbar','viewHost',
    'workbench','workbenchResize','workbenchSubtitle','closeWorkbench','workbenchTabs','workbenchBody','alternativesTray','alternativesTrayContext','alternativesTrayBody','closeAlternativesTray','demoGuide','demoGuideProgress','demoGuideBody','closeDemoGuide','demoGuideBack','demoGuideNext','recordingTransport','toast','overlayLayer'
  ].map(id => [id, $(id)]));
  ...
  function createLine(text = '', kind = 'lyric', key = 'G') {
    ...
    return { id:uid('line'), text:parsed.text, kind:'lyric', chords:parsed.chords, syllableOverride:null, attention:false, style:{}, shape:null };
  }
  ...
  function createSong(title = 'Untitled Song') {
    ...
    return {
      ...
      timeline:[], takes:[], exchangeHistory:[], sourceSongId:null,
      format:{fontFamily:'serif',textColor:'ink',fontSize:19,lineHeight:1.65,pageWidth:860}
    };
  }
...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  const DEMO_STEPS = [
    {view:'plan',title:'Move an idea out of the page',copy:'Select a useful phrase in Plan and choose Send to Shape. The source stays here; a loose fragment appears on the desk.',target:'.brain-dump'},
    {view:'shape',title:'Arrange the pieces',copy:'Drag “The porch light stayed on…” anywhere on the desk. Select moves pieces; Hand or Space-drag pans the board.',target:'[data-demo-key="porch-light"]'},
    {view:'shape',title:'Place a line in the song',copy:'Select a loose fragment and choose Place in song. It becomes a ruled lyric row in the active section.',target:'[data-demo-key="porch-light"]'},
    {view:'write',title:'Write and add a chord',copy:'Use the blank row in Verse 2, then place a chord above a word with + Chord or bracket entry such as [G].',target:'[data-demo-key="blank-demo-row"]'},
    {view:'write',title:'Try another line',copy:'Select a lyric row and choose Try another. The old wording moves to Shelf → Alternatives while the page stays ready to write.',target:'.notebook-row'},
    {view:'write',title:'Move the harmony',copy:'Transpose the Chorus from its section menu. Then open the shapes control: capo and shapes should change without moving the sounding chords.',target:'.notebook-meta'},
    {view:'chart',title:'Open the chart',copy:'Switch between chord names, Nashville numbers and lyrics-only, then open Export to preview the finished hand-off.',target:'.chart-page'}
  ];
  ...
  function normaliseSong(raw) {
    ...
    song.format = {fontFamily:'serif',textColor:'ink',fontSize:19,lineHeight:1.65,pageWidth:860,...(song.format || {})};
    ...
    song.format.pageWidth = clamp(Number(song.format.pageWidth)||860,640,1080);
    song.lastView = ['plan','shape','write','chart'].includes(song.lastView) ? song.lastView : (song.sections.length ? 'write' : 'plan');
    song.viewMemory ||= {};
  ...
  function normaliseState(raw) {
    ...
    result.shellPage = ['welcome','library','workspace'].includes(result.shellPage) ? result.shellPage : (result.selectedSongId?'workspace':'welcome');
  ...
  function undo() { const item = undoStack.pop(); if (!item) return; redoStack.push({label:item.label,runtime:runtimeSnapshot()}); restoreRuntime(item.runtime); resetActiveSelection(); scheduleSave(); render(); toast(`${item.label} undone`); updateUndoButtons(); }
  function redo() { const item = redoStack.pop(); if (!item) return; undoStack.push({label:item.label,runtime:runtimeSnapshot()}); restoreRuntime(item.runtime); resetActiveSelection(); scheduleSave(); render(); toast(`${item.label} restored`); updateUndoButtons(); }
  ...
  function perform(label,mutate,message=label,after=null) { snapshot(label); mutate(); const song = currentSong(); if (song) touch(song); render(); toast(message,undo); after?.(); }

  function resetActiveSelection() { activeLineId=null; activeSectionId=null; activeChordId=null; activeChordLineId=null; activeProgressionToken=null; pendingChordAnchor=null; activeEditor=null; shapeSelection.clear(); shapeInteraction=null; stopShapeAutoPan(); closeSelectionBar(); closeOverlay(); }
  ...
  async function startWorkTape(song=null,source='workspace'){
    ...
    try{
      ...
      recordingTarget={songId:song?.id||null,view:song?state.view:'welcome',source,takeId:uid('take')};
  ...
  async function finishWorkTape(){
    ...
    if(song){song.takes.unshift(take);song.timeline.unshift({id:uid('log'),type:'take',takeId:take.id,createdAt:take.createdAt,view:take.view});touch(song);toast('Work Tape saved');openStudioLog(song);}
    else{state.unattachedTakes.unshift(take);scheduleSave();renderWelcome();toast('Idea saved — choose where it belongs');openAttachCapture(take);}
  }

  function attachTakeToSong(take,song){state.unattachedTakes=state.unattachedTakes.filter(item=>item.id!==take.id);song.takes.unshift(take);song.timeline.unshift({id:uid('log'),type:'take',takeId:take.id,createdAt:take.createdAt,view:take.view||'welcome'});touch(song);render();toast(`Attached to ${song.title}`);}
  function openAttachCapture(take){
    const choices=state.songs.map(song=>[song.title,()=>attachTakeToSong(take,song)]);
    choices.unshift(['Create a new song…',()=>openFormModal('New song for this idea',[{name:'title',label:'Song title',type:'text',value:'Untitled Song',required:true}],values=>{const song=createSong(values.title.trim());state.songs.unshift(song);state.selectedSongId=song.id;attachTakeToSong(take,song);setTimeout(()=>openStudioLog(song));})]);
    choices.push(['Keep in “Needs a home”',()=>{scheduleSave();render();}]);
    openChoiceModal('Where does this idea belong?',choices,'The recording is already saved locally. You can safely decide later.');
  }

  function openUnattachedCaptures(){
    const body=document.createElement('div');body.className='studio-log-list';
    state.unattachedTakes.forEach(take=>{const row=document.createElement('article');row.className='studio-log-entry unattached';const meta=document.createElement('div');meta.innerHTML=`<strong>${escapeHtml(take.name)}</strong><small>${escapeHtml(formatDuration(take.durationMs))} · ${escapeHtml(relativeDate(take.createdAt))}</small>`;const audioWrap=document.createElement('div');audioWrap.className='studio-log-audio';const actions=document.createElement('div');actions.className='studio-log-actions';const attach=document.createElement('button');attach.className='ghost-button';attach.textContent='Attach';attach.onclick=()=>{closeOverlay();openAttachCapture(take);};const download=document.createElement('button');download.className='text-button';download.dataset.download='';download.textContent='Download';const remove=document.createElement('button');remove.className='text-button danger';remove.textContent='Delete';remove.onclick=async()=>{if(!confirm(`Delete “${take.name}”? This removes the local audio.`))return;await audioDelete(take.id);state.unattachedTakes=state.unattachedTakes.filter(item=>item.id!==take.id);scheduleSave();openUnattachedCaptures();toast('Capture deleted');};actions.append(attach,download,remove);row.append(meta,audioWrap,actions);body.append(row);populateTapeAudio(row,take);});
    openModal('Needs a home',body);
  }

  async function populateTapeAudio(row,take){
    try{const blob=await audioGet(take.id);if(!blob){row.classList.add('audio-missing');return;}const audio=document.createElement('audio');audio.controls=true;audio.preload='metadata';const url=URL.createObjectURL(blob);studioLogUrls.push(url);audio.src=url;row.querySelector('.studio-log-audio')?.append(audio);const download=row.querySelector('[data-download]');if(download)download.onclick=()=>downloadBlob(`${safeName(take.name)}.${take.mimeType?.includes('mp4')?'m4a':'webm'}`,blob);}
  ...
  function applyFormatVariables(song) {
    const font = song.format.fontFamily === 'sans' ? '"Helvetica Neue",Helvetica,Arial,sans-serif' : song.format.fontFamily === 'typewriter' ? '"Courier Prime","Courier New",Courier,monospace' : 'Georgia,"Times New Roman",serif';
  ...
  function render() {
    ...
    els.songContext.innerHTML = `${inSandboxMode()?'<span class="demo-badge sandbox-badge">SANDBOX</span>':inDemoMode()?'<span class="demo-badge">DEMO</span>':''}`;
    renderToolbar(song);
    renderView(song);
    renderWorkbench();
  ...
  function renderWelcome(){
    ...
    els.welcomeRecordIdea.textContent=mediaRecorder?'● Recording…':'● Record an idea';els.welcomeRecordIdea.disabled=Boolean(mediaRecorder);
  }
  ...
  function renderLibrary() {
    ...
    renderProjects();
    if(!inDemoMode()&&state.unattachedTakes.length){const pending=document.createElement('section');pending.className='unattached-captures';pending.innerHTML=`<div><strong>Needs a home</strong><span>${state.unattachedTakes.length} saved idea${state.unattachedTakes.length===1?'':'s'}</span></div>`;const open=document.createElement('button');open.className='ghost-button';open.textContent='Review';open.onclick=openUnattachedCaptures;pending.append(open);els.songList.append(pending);}
  ...
  function setView(view) {
    ...
    song.viewMemory[state.view] ||= {}; song.viewMemory[state.view].scrollTop = els.viewHost.scrollTop;
    state.view=view;song.lastView=view;state.shellPage='workspace';if(view==='chart'){state.workbenchOpen=false;state.alternativesTrayOpen=false;}else if(view!=='write')state.alternativesTrayOpen=false;closeSelectionBar();closeOverlay();activeEditor=null;pendingChordAnchor=null;activeChordId=null;activeChordLineId=null;activeProgressionToken=null;scheduleSave();render();
    requestAnimationFrame(()=>{const memory=song.viewMemory?.[view];if(memory?.scrollTop!==undefined)els.viewHost.scrollTop=memory.scrollTop;if(view==='plan'&&!song.plan.brainDump)document.querySelector('.brain-dump')?.focus();});
  }
  ...
  function openTextMenu(song,anchor,context){
    ...
    styles.controls.append(bold,italic,clean);panel.append(fonts.wrap,appearance.wrap,styles.wrap);
    placePopover(panel,rect,{width:560,height:176,prefer:'below'});
  ...
  function renderToolbar(song) {
    ...
    if (state.view==='plan') {
      left.append(toolButton('Send to Shape','Copy the selection or current paragraph to Shape',()=>sendPlanToShape(song),'primary-action'));right.append(toolButton('Text','Point size and restrained formatting',event=>openTextMenu(song,event.currentTarget,'plan')),toolButton('Shelf','Open words or music',()=>toggleWorkbench(state.workbenchTab||'words'),state.workbenchOpen?'active':''));
    } else if (state.view==='shape') {
      left.append(
        toolButton('Select','Select and move pieces',()=>setShapeTool('select',song),state.shapeTool==='select'?'active':''),
        toolButton('Hand','Pan the desk',()=>setShapeTool('hand',song),state.shapeTool==='hand'?'active':''),
        divider(),toolButton('+ Fragment','Create a loose fragment',()=>addShapeBlock(song,'fragment'),'primary-action'),
        toolButton('+ Section','Create a movable section',event=>openSectionPicker(song,song.sections.length,'',false,event.currentTarget))
      );
      right.append(toolButton('Shelf','Open words or music',()=>toggleWorkbench(state.workbenchTab||'words'),state.workbenchOpen?'active':''));
      if(shapeSelection.size){const selected=[...shapeSelection].map(parseShapeKey);const onlySection=selected.length===1&&selected[0].kind==='section';const onlyLines=selected.every(item=>item.kind==='line');if(onlyLines)right.append(toolButton('Lift to desk','Detach the selected lines from their section',()=>liftSelectedShapeLines(song),'selection-action'));else right.append(toolButton(onlySection?'Open in Write':'Place in song','Place selected material in the notebook',()=>sendShapeSelectionToWrite(song),'selection-action'));}
    } else if (state.view==='write') {
      left.append(toolButton('+ Section','Add a section after the active section',event=>openSectionPicker(song,sectionInsertIndex(song),'',true,event.currentTarget),'primary-action'),toolButton('+ Chord','Place a chord above the active word',()=>addChordFromToolbar(song)),toolButton('Try another','Preserve the current choice and try another',()=>tryAnother(song)),toolButton('Shelf','Open words or music',()=>toggleWorkbench(state.workbenchTab||'words'),state.workbenchOpen?'active':''));
      right.append(toolButton('Text','Point size and restrained formatting',event=>openTextMenu(song,event.currentTarget,'write')));
    } else {
      left.append(toolSelect(state.chartMode,[{value:'shapes',label:'Playing shapes'},{value:'sounding',label:'Sounding chords'},{value:'numbers',label:'Nashville numbers'},{value:'dual',label:'Shapes + sounding'},{value:'lyrics',label:'Lyrics only'},{value:'progressions',label:'Progressions'}],value=>{state.chartMode=value;scheduleSave();renderView(song);},'Chart view'),toolButton('Key/Capo','Sounds in, shapes, capo, tuning and player tools',()=>openKeyCapoPanel(song)),toolButton('Layout','Chart font size and spacing',()=>openChartLayout(song)),toolButton('Export','Print, text or ChordPro',()=>openChartExport(song),'primary-action'));
    }
  ...
  function sendPlanToShape(song) { const selected=selectedPlanText();if(!selected?.text){toast('Write or select something first');return;}const material=planSelectionMaterial(selected.text),center=visibleShapeCenter(song);perform('Copy to Shape',()=>{let index=0;material.forEach(item=>{const x=center.x+(index%3)*235,y=center.y+Math.floor(index/3)*130;if(item.kind==='section'){const section=createSection(item.label,item.lines.map(line=>createLine(line,'lyric',song.key)),x,y);section.shape.h=Math.max(180,70+item.lines.length*68);song.sections.push(section);}else song.shapeBlocks.push(createBlock('fragment',item.text,x,y));index+=1;});},material.length===1?'Copied to Shape':`${material.length} pieces copied to Shape`); }
  ...
  function renderShapeBlock(song,block) {
    ...
    const head=document.createElement('header');head.className='shape-block-head';const grip=document.createElement('button');grip.className='shape-grip';grip.textContent='⋮⋮';grip.title='Move fragment';grip.onpointerdown=event=>beginShapeObjectPointer(event,song,key,true);const label=document.createElement('span');label.className='shape-label';label.textContent=block.type==='harmony'?'Progression':block.type==='title'?'Title':'';const more=document.createElement('button');more.className='shape-more';more.textContent='•••';more.onclick=event=>{event.stopPropagation();openShapeBlockMenu(song,block,more);};head.append(grip,label,more);article.append(head);
    if(shapeEditingKey===key){const textarea=document.createElement('textarea');textarea.value=block.text;textarea.placeholder='Fragment';textarea.onpointerdown=event=>event.stopPropagation();textarea.oninput=()=>{block.text=textarea.value;autoGrow(textarea,62);requestAnimationFrame(()=>{block.h=Math.max(88,article.offsetHeight);touch(song);});};textarea.onblur=()=>{shapeEditingKey=null;block.h=Math.max(88,article.offsetHeight);touch(song);renderView(song);};textarea.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();textarea.blur();}};article.append(textarea);requestAnimationFrame(()=>{autoGrow(textarea,62);textarea.focus();textarea.selectionStart=textarea.selectionEnd=textarea.value.length;});}
    else{const copy=document.createElement('div');copy.className=`shape-copy${block.type==='harmony'?' progression-copy':''}`;copy.textContent=block.type==='harmony'?displayProgression(block.text,song,state.chordDisplay,song.key):(block.text||'Blank fragment');article.append(copy);}
  ...
  function renderShapeSection(song,section) {
    ...
    const head=document.createElement('header');head.className='shape-section-head';const grip=document.createElement('button');grip.className='shape-grip';grip.textContent='⋮⋮';grip.title='Move section';grip.onpointerdown=event=>beginShapeObjectPointer(event,song,key,true);const title=document.createElement('div');title.className='shape-section-title';title.textContent=section.label;title.onpointerdown=event=>beginShapeObjectPointer(event,song,key,true);title.ondblclick=event=>{event.preventDefault();event.stopPropagation();openFormModal('Rename section',[{name:'label',label:'Section name',type:'text',value:section.label,required:true}],values=>{section.label=values.label.trim();touch(song);render();});};const more=document.createElement('button');more.className='shape-more';more.textContent='•••';more.onclick=e=>{e.stopPropagation();openSectionMenu(song,section,more);};head.append(grip,title,more);
    ...
    if(!section.lines.length){const empty=document.createElement('div');empty.className='shape-empty';empty.textContent='Drop a fragment here';canvas.append(empty);}
  ...
  function renderShapeSectionLine(song,section,line){
    ...
    else{const copy=document.createElement('div');copy.className='shape-line-copy';copy.textContent=line.kind==='progression'?displayProgression(line.text,song,state.chordDisplay,sectionKey(song,section)):(line.text||'Blank line');chip.append(copy);const more=document.createElement('button');more.className='shape-line-more';more.textContent='•••';more.title='Line options';more.onclick=event=>{event.stopPropagation();openMenu(more,[['Lift to desk',()=>liftLineToShape(song,section,line,section.shape.x+line.shape.x,section.shape.y+SHAPE_SECTION_HEADER+line.shape.y)],['Open in Write',()=>{activeSectionId=section.id;activeLineId=line.id;setView('write');requestAnimationFrame(()=>document.querySelector(`[data-line-id="${CSS.escape(line.id)}"]`)?.scrollIntoView({block:'center'}));}],['Delete',()=>perform('Deleted line',()=>section.lines=section.lines.filter(item=>item.id!==line.id),'Line deleted'),'danger']]);};chip.append(more);}
  ...
  function shapePointerUp(event){
    ...
    if(interaction.type==='object-pending'){updateShapeSelectionUI(song);return;}
  ...
  function addShapeBlock(song,type='fragment',point=null){const center=point||visibleShapeCenter(song);snapshot('Created Shape fragment');const block=createBlock(type,'',center.x,center.y);song.shapeBlocks.push(block);shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));rememberShapeSelection(song);touch(song);shapeEditingKey=shapeKey('block',block.id);render();}
  function liftLineToShape(song,section,line,x,y){perform('Lifted line to Shape',()=>{const block=createBlock(line.kind==='progression'?'harmony':'fragment',line.text,x,y,line.chords);block.sourceLineId=line.id;block.w=line.shape?.w||220;block.h=line.shape?.h||96;song.shapeBlocks.push(block);section.lines=section.lines.filter(item=>item.id!==line.id);shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));rememberShapeSelection(song);},'Line lifted to the desk');}
  function liftSelectedShapeLines(song){const lines=[...shapeSelection].map(parseShapeKey).filter(item=>item.kind==='line').map(item=>shapeLineOwner(song,item.id)).filter(Boolean);if(!lines.length)return;perform('Lifted lines to Shape',()=>{const next=new Set(shapeSelection);lines.forEach(({section,line})=>{const box=shapeAbsoluteBox(song,shapeKey('line',line.id));const block=createBlock(line.kind==='progression'?'harmony':'fragment',line.text,box.x,box.y,line.chords);block.sourceLineId=line.id;block.w=box.w;block.h=box.h;song.shapeBlocks.push(block);section.lines=section.lines.filter(item=>item.id!==line.id);next.delete(shapeKey('line',line.id));next.add(shapeKey('block',block.id));});shapeSelection=next;rememberShapeSelection(song);},lines.length===1?'Line lifted to the desk':'Lines lifted to the desk');}
  ...
  function openShapeBlockMenu(song,block,anchor){
    ...
    openMenu(anchor,[['Edit fragment',()=>{shapeEditingKey=shapeKey('block',block.id);shapeSelection.clear();shapeSelection.add(shapeEditingKey);rememberShapeSelection(song);renderView(song);}],['Place in song',()=>{shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));sendShapeSelectionToWrite(song);}],...(destinations.length?[['Place in section…',()=>openChoiceModal('Place in section',destinations)]]:[]),['Convert to fragment',()=>{block.type='fragment';touch(song);render();}],['Convert to title',()=>{block.type='title';touch(song);render();}],['Convert to progression',()=>{block.type='harmony';touch(song);render();}],['Delete',()=>perform('Deleted fragment',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Fragment deleted'),'danger']]);
  }
  function addShapeBlockToSection(song,block,section){perform('Place fragment in section',()=>{const line=createLine(block.text,block.type==='harmony'?'progression':'lyric',sectionKey(song,section));if(block.sourceLineId)line.id=block.sourceLineId;if(block.chords?.length&&line.kind==='lyric')line.chords=clone(block.chords);line.shape={x:12,y:12+section.lines.length*68,w:Math.max(180,section.shape.w-24),h:line.kind==='progression'?52:58};section.lines.push(line);sortSectionLinesByShape(section);expandSectionToContents(section);song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id);shapeSelection.delete(shapeKey('block',block.id));shapeSelection.add(shapeKey('line',line.id));rememberShapeSelection(song);activeSectionId=section.id;activeLineId=line.id;},`Added to ${section.label}`);}

  function sendShapeSelectionToWrite(song){
    const selected=[...shapeSelection];if(!selected.length){toast('Select a fragment or section first');return;}
  ...
  function renderWrite(song) {
    ...
    const profile=activeProfile(song);const meta=document.createElement('button');meta.className='notebook-meta';meta.textContent=`${compactKey(song.key)} · ${profile.shapeKey} shapes${profile.capo?` · capo ${profile.capo}`:' · no capo'} · ${profile.tuning} · ${displayModeLabel(state.chordDisplay)}`;meta.onclick=()=>openNotebookMetaMenu(song,meta);
    ...
    page.append(meta,sections);wrap.append(page);view.append(sticky,wrap);els.viewHost.append(view);
  }
  ...
  function renderSectionInsert(song,index){const wrap=document.createElement('div');wrap.className='section-insert';const button=document.createElement('button');button.textContent='+';button.title='Add section here';button.onclick=()=>openSectionPicker(song,index,'',true,button);wrap.append(button);return wrap;}
  ...
  function renderWriteSection(song,section){
    ...
    const head=document.createElement('header');head.className='write-section-head';const grip=document.createElement('button');grip.className='section-grip';grip.textContent='⋮⋮';grip.draggable=true;grip.ondragstart=event=>{dragData={kind:'section',id:section.id};event.dataTransfer.setData('text/plain',section.id);};grip.ondragend=()=>dragData=null;const label=document.createElement('input');label.className='write-section-label';label.value=section.label;label.onfocus=()=>{activeSectionId=section.id;};label.oninput=()=>{section.label=label.value;touch(song);renderLibrary();};const actions=document.createElement('div');actions.className='section-actions';const collapse=document.createElement('button');collapse.textContent=section.collapsed?'⌄':'⌃';collapse.title=section.collapsed?'Open section':'Close section';collapse.onclick=()=>{section.collapsed=!section.collapsed;touch(song);renderView(song);};const more=document.createElement('button');more.textContent='•••';more.onclick=()=>openSectionMenu(song,section,more);actions.append(collapse,more);head.append(grip,label);if(section.keyOverride){const note=document.createElement('span');note.className='section-key-note';note.textContent=section.keyOverride;head.append(note);}head.append(actions);article.append(head);
    article.ondragover=event=>{if(dragData?.kind==='section'){event.preventDefault();article.classList.add('drop-section');}};article.ondrop=event=>{if(dragData?.kind!=='section')return;event.preventDefault();const from=song.sections.findIndex(item=>item.id===dragData.id),to=song.sections.findIndex(item=>item.id===section.id);if(from!==to)perform('Moved section',()=>{const [moved]=song.sections.splice(from,1);song.sections.splice(to,0,moved);},'Section moved');dragData=null;};
  ...
  function renderNotebookRow(song,section,line,index){
    const row=document.createElement('div');row.className=`notebook-row${line.id===activeLineId?' active':''}${line.kind==='progression'?' chord-row':''}`;row.dataset.lineId=line.id;if(line.demoKey)row.dataset.demoKey=line.demoKey;
    const grip=document.createElement('button');grip.className='line-grip';grip.textContent='⋮⋮';grip.draggable=true;grip.title='Move row';grip.setAttribute('aria-label','Move row');grip.ondragstart=event=>{dragData={kind:'line',lineId:line.id,sectionId:section.id};event.dataTransfer.setData('text/plain',line.id);event.dataTransfer.effectAllowed='move';};grip.ondragend=()=>{dragData=null;row.classList.remove('drop-before','drop-after');};
    ...
    const right=document.createElement('div');right.className='line-right';const count=lineAlternativeCount(song,line);if(count){const alt=document.createElement('button');alt.className='alt-count';alt.textContent=`${count} alternative${count===1?'':'s'}`;alt.onclick=event=>{event.stopPropagation();activeLineId=line.id;activeSectionId=section.id;openAlternativesTray();};right.append(alt);}const menu=document.createElement('button');menu.className='line-menu-button';menu.textContent='•••';menu.title='Row menu';menu.onclick=event=>{event.stopPropagation();openLineMenu(song,section,line,menu);};right.append(menu);row.append(grip,paper,right);
    ...
    row.ondragover=event=>{if(!['line','shelf-fragment','shelf-chord','shelf-progression'].includes(dragData?.kind))return;event.preventDefault();const rect=row.getBoundingClientRect();row.classList.toggle('drop-before',event.clientY<rect.top+rect.height/2);row.classList.toggle('drop-after',event.clientY>=rect.top+rect.height/2);};row.ondragleave=()=>row.classList.remove('drop-before','drop-after');row.ondrop=event=>{if(!dragData)return;event.preventDefault();const rect=row.getBoundingClientRect(),targetIndex=index+(event.clientY>=rect.top+rect.height/2?1:0);if(dragData.kind==='line')moveLine(song,dragData.sectionId,dragData.lineId,section.id,targetIndex);else if(dragData.kind==='shelf-fragment'){const block=song.shapeBlocks.find(item=>item.id===dragData.blockId);if(block)perform('Created lyric alternative',()=>{song.alternatives.unshift({id:uid('alt'),type:'lyric',targetType:'line',targetId:line.id,label:`Option ${String.fromCharCode(66+lineAlternativeCount(song,line))}`,content:{line:{text:block.text,chords:clone(block.chords||[]),kind:'lyric',style:{}}},createdAt:now()});state.alternativesTrayOpen=true;activeLineId=line.id;activeSectionId=section.id;},'Fragment added as an alternative');}else if(dragData.kind==='shelf-chord'){if(line.kind==='lyric')perform('Added chord',()=>line.chords.push({id:uid('chord'),value:dragData.value,anchor:NE.nearestBoundary(line.text,line.text.length,true)}),'Chord added to line');}else if(dragData.kind==='shelf-progression'){perform('Added chord row',()=>section.lines.splice(targetIndex,0,createLine(dragData.text,'progression',sectionKey(song,section))),'Progression added to section');}dragData=null;};return row;
  }
  ...
  function renderChordRow(song,section,line){
    ...
    bars.forEach((bar,barIndex)=>{const cell=document.createElement('span');cell.className='chord-bar';cell.dataset.bar=String(barIndex);const values=bar.split(/\s+/).filter(Boolean);if(!values.length){const blank=document.createElement('span');blank.className='chord-row-blank';blank.textContent=' ';cell.append(blank);}values.forEach((rawValue,chordIndex)=>{const canonical=CE.parseNashville(rawValue)?CE.nashvilleToChord(rawValue,sectionKey(song,section),song.spelling):rawValue;const token=document.createElement('button');token.className=`chord-row-token${activeProgressionToken?.lineId===line.id&&activeProgressionToken.barIndex===barIndex&&activeProgressionToken.chordIndex===chordIndex?' selected':''}`;token.draggable=true;token.dataset.bar=String(barIndex);token.dataset.chord=String(chordIndex);const main=document.createElement('span');main.textContent=storedChordToDisplay(canonical,song,state.chordDisplay,sectionKey(song,section));token.append(main);if(profile.showSounding&&state.chordDisplay==='shapes'){const secondary=document.createElement('small');secondary.textContent=canonical;token.append(secondary);}if(profile.showNashville&&state.chordDisplay!=='numbers'){const number=document.createElement('small');number.textContent=CE.chordToNashville(canonical,sectionKey(song,section));token.append(number);}token.onpointerdown=event=>event.stopPropagation();token.onclick=event=>{event.preventDefault();event.stopPropagation();activeLineId=line.id;activeSectionId=section.id;activeChordId=null;activeChordLineId=null;activeProgressionToken={lineId:line.id,barIndex,chordIndex,value:canonical};pendingChordAnchor=null;toggleWorkbench('music');renderView(song);};token.ondragstart=event=>{event.stopPropagation();dragData={kind:'progression-chord',lineId:line.id,barIndex,chordIndex};event.dataTransfer.setData('text/plain',canonical);event.dataTransfer.effectAllowed='move';};token.ondragend=()=>dragData=null;cell.append(token);});cell.ondragover=event=>{if(dragData?.kind==='progression-chord'&&dragData.lineId===line.id){event.preventDefault();cell.classList.add('drop-target');}};cell.ondragleave=()=>cell.classList.remove('drop-target');cell.ondrop=event=>{if(dragData?.kind!=='progression-chord'||dragData.lineId!==line.id)return;event.preventDefault();cell.classList.remove('drop-target');const source={...dragData};perform('Moved chord in chord row',()=>{moveProgressionToken(line,source,barIndex);activeProgressionToken=null;},'Chord moved');dragData=null;};container.append(cell);});
  ...
  function wordRanges(text){const source=String(text||'');const starts=NE.wordStarts(source);const ranges=[];for(let index=0;index<starts.length-1;index++)ranges.push({start:starts[index],end:starts[index+1],text:source.slice(starts[index],starts[index+1])});if(!ranges.length)ranges.push({start:0,end:0,text:''});if((source.length===0||source.trimEnd()===source)&&!ranges.some(range=>range.start===source.length))ranges.push({start:source.length,end:source.length,text:''});return ranges;}
  ...
  function renderChordToken(song,section,line,chord){const button=document.createElement('button');button.className=`chord-token${chord.id===activeChordId?' selected':''}${chord.needsReview?' needs-review':''}`;button.title=chord.needsReview?'Check chord position':'Select chord';button.draggable=true;button.setAttribute('aria-label',`Chord ${storedChordToDisplay(chord.value,song,state.chordDisplay,sectionKey(song,section))}`);const main=document.createElement('span');main.textContent=storedChordToDisplay(chord.value,song,state.chordDisplay,sectionKey(song,section));button.append(main);const profile=activeProfile(song);if(profile.showSounding&&state.chordDisplay==='shapes'){const secondary=document.createElement('small');secondary.textContent=chord.value;button.append(secondary);}if(profile.showNashville&&state.chordDisplay!=='numbers'){const number=document.createElement('small');number.textContent=CE.chordToNashville(chord.value,sectionKey(song,section));button.append(number);}button.onpointerdown=event=>event.stopPropagation();button.ondragstart=event=>{event.stopPropagation();dragData={kind:'chord',lineId:line.id,chordId:chord.id};event.dataTransfer.setData('text/plain',chord.id);event.dataTransfer.effectAllowed='move';};button.ondragend=()=>dragData=null;button.onclick=event=>{event.preventDefault();event.stopPropagation();activeLineId=line.id;activeSectionId=section.id;activeChordId=chord.id;activeChordLineId=line.id;activeProgressionToken=null;pendingChordAnchor=null;toggleWorkbench('music');renderView(song);};return button;}

  function openLineEditor(song,section,line,cursor=null){activeLineId=line.id;activeSectionId=section.id;activeChordId=null;activeChordLineId=null;activeProgressionToken=null;pendingChordAnchor=null;const existing=document.querySelector(`.notebook-row[data-line-id="${CSS.escape(line.id)}"] .line-editor`);if(existing){existing.focus();if(cursor!==null)existing.selectionStart=existing.selectionEnd=clamp(cursor,0,existing.value.length);return;}if(activeEditor)activeEditor.blur();const row=document.querySelector(`.notebook-row[data-line-id="${CSS.escape(line.id)}"]`);if(!row)return;row.classList.add('active');const paper=row.querySelector('.line-paper');const display=paper.querySelector('.notebook-content,.chord-row-content');if(!display)return;const textarea=document.createElement('textarea');textarea.className=`line-editor${line.kind==='progression'?' chord-row-editor':''}`;textarea.value=line.kind==='progression'?displayProgression(line.text,song,state.chordDisplay,sectionKey(song,section)):line.text;display.replaceWith(textarea);activeEditor=textarea;textarea.focus();textarea.selectionStart=textarea.selectionEnd=cursor===null?textarea.value.length:clamp(cursor,0,textarea.value.length);autoGrow(textarea,42);
    textarea.oninput=()=>{if(line.kind==='progression')line.text=displayedProgressionToStored(textarea.value,song,sectionKey(song,section));else consumeLyricInput(song,section,line,textarea);touch(song);autoGrow(textarea,42);};
    ...
    textarea.onblur=()=>{if(activeEditor===textarea)activeEditor=null;touch(song);renderView(song);renderWorkbench();};
  }
  ...
  function moveLineToSpare(song,section,line){perform('Moved line to Spare Parts',()=>{const block=createBlock(line.kind==='progression'?'harmony':'fragment',line.text,section.shape.x+section.shape.w+40,section.shape.y+50,line.chords);block.sourceLineId=line.id;song.shapeBlocks.push(block);section.lines=section.lines.filter(item=>item.id!==line.id);activeLineId=null;},'Line moved to Spare Parts');}
  ...
  function renderWorkbench(){
    ...
    if(transientPanel){
      title.textContent=transientPanel.type==='transpose'?'Transpose':transientPanel.type==='interpret'?'Set key':transientPanel.type==='keycapo'?'Player setup':'Shelf';
    ...
    title.textContent='Shelf';els.workbenchTabs.classList.remove('hidden');els.workbenchSubtitle.textContent='';[...els.workbenchTabs.querySelectorAll('button')].forEach(button=>button.classList.toggle('active',button.dataset.tab===state.workbenchTab));els.workbenchBody.replaceChildren();
  ...
  function alternativeTargetLabel(song,alt){if(alt.targetType==='song')return song.title;if(alt.targetType==='section')return findSection(song,alt.targetId)?.label||'Missing section';if(alt.targetType==='chord'){const found=findChord(song,alt.parentLineId,alt.targetId);return found?`${found.section.label} · chord`:'Unplaced chord';}const found=findLine(song,alt.targetId);if(found)return`${found.section.label} · line ${found.section.lines.indexOf(found.line)+1}`;const spare=song.shapeBlocks.find(block=>block.sourceLineId===alt.targetId);return spare?'Spare Part':'Unplaced alternative';}
  function renderAlternativeCard(song,alt){const card=document.createElement('article');card.className=`shelf-card${alt.type==='harmony'?' harmony':alt.type==='title'?' title':''}`;card.innerHTML=`<div class="shelf-meta">${escapeHtml(alternativeTargetLabel(song,alt))}</div><div class="shelf-copy">${escapeHtml(alternativePreview(alt))}</div>`;const actions=document.createElement('div');actions.className='shelf-actions';const use=document.createElement('button');use.textContent='Use';use.onclick=()=>activateAlternative(song,alt);const compare=document.createElement('button');compare.textContent='Compare';compare.onclick=()=>openAlternativeCompare(song,alt);const more=document.createElement('button');more.textContent='•••';more.onclick=()=>openMenu(more,[['Move to Spare Parts',()=>alternativeToSpare(song,alt)],['Delete',()=>perform('Deleted alternative',()=>song.alternatives=song.alternatives.filter(item=>item.id!==alt.id),'Alternative deleted'),'danger']]);actions.append(use,compare,more);card.append(actions);return card;}
  ...
  function renderAlternativesTray(){
    ...
    els.alternativesTrayContext.textContent=activeChordId?'Selected chord':found?`${found.section.label} · line ${found.section.lines.indexOf(found.line)+1}`:activeSectionId?(findSection(song,activeSectionId)?.label||'Selected section'):'Select a line, chord or section';
  ...
  function alternativeToSpare(song,alt){perform('Moved alternative to Spare Parts',()=>{if(alt.content?.line){song.shapeBlocks.push(createBlock(alt.content.line.kind==='progression'?'harmony':'fragment',alt.content.line.text,180,140,alt.content.line.chords));}else song.shapeBlocks.push(createBlock(alt.type==='harmony'?'harmony':alt.type==='title'?'title':'fragment',alternativePreview(alt),180,140));song.alternatives=song.alternatives.filter(item=>item.id!==alt.id);},'Moved to Spare Parts');}

  function renderWorkbenchSpare(song){
    const add=document.createElement('button');add.className='shelf-add';add.textContent='+ Spare part';
    add.onclick=()=>{const center=visibleShapeCenter(song);song.shapeBlocks.push(createBlock('fragment','',center.x,center.y));touch(song);if(state.view==='shape')renderView(song);renderWorkbench();};
    ...
    song.shapeBlocks.forEach(block=>{
      ...
      card.innerHTML=`${block.type==='harmony'||block.type==='title'?`<div class="shelf-meta">${escapeHtml(block.type==='harmony'?'Progression':'Title')}</div>`:''}<div class="shelf-copy">${escapeHtml(block.text||'Blank fragment')}</div>`;
      const actions=document.createElement('div');actions.className='shelf-actions';
      const use=document.createElement('button');use.textContent=state.view==='plan'?'Copy to Plan':state.view==='shape'?'Place on desk':'Place in song';
      use.onclick=()=>{
        if(state.view==='plan'){
        ...
        }else insertSpareBlock(song,block);
      };
      const secondary=document.createElement('button');secondary.textContent=state.view==='shape'?'Place in song':'Place in Shape';
      secondary.onclick=()=>{if(state.view==='shape')insertSpareBlock(song,block);else{shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));setView('shape');}};
      const more=document.createElement('button');more.textContent='•••';
      more.onclick=()=>openMenu(more,[['Copy',()=>{const copy=clone(block);copy.id=uid('block');copy.x+=28;copy.y+=28;song.shapeBlocks.push(copy);touch(song);renderWorkbench();}],['Delete',()=>perform('Deleted spare part',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Spare part deleted'),'danger']]);
      actions.append(use,secondary,more);card.append(actions);stack.append(card);
    });
    if(!song.shapeBlocks.length){const empty=document.createElement('div');empty.className='workbench-empty';empty.textContent='Unused lines, titles and notes appear here.';stack.append(empty);}
    els.workbenchBody.append(stack);
  }

  function renderWorkbenchChords(song){const context=chordContext(song);const profile=activeProfile(song);const key=context?sectionKey(song,context.section):song.key;const info=document.createElement('div');info.className='workbench-empty';const representation=state.chordDisplay==='shapes'?(tuningIsAutomatic(profile.tuning)?`In ${profile.shapeKey} shapes`:'Manual chord names'):state.chordDisplay==='numbers'?'Nashville numbers':`In ${compactKey(key)} sounding chords`;info.innerHTML=`<strong>${escapeHtml(representation)}</strong><br>${context?.mode==='replace'?`Choose a replacement for ${escapeHtml(storedChordToDisplay(context.chord.value,song,state.chordDisplay,key))}.`:context?.mode==='progression-replace'?`Choose a replacement for ${escapeHtml(storedChordToDisplay(context.token.value,song,state.chordDisplay,key))}.`:context?'Add a chord above this word.':'Select a lyric position, chord or chord row.'}`;els.workbenchBody.append(info);
    const addSection=(label,items)=>{if(!items.length)return;const heading=document.createElement('div');heading.className='shelf-section-heading';heading.textContent=label;els.workbenchBody.append(heading);const grid=document.createElement('div');grid.className='chord-helper-grid';items.forEach(item=>{const canonical=item.chord||item;const button=document.createElement('button');button.className='chord-choice';button.draggable=true;button.innerHTML=`<span>${escapeHtml(storedChordToDisplay(canonical,song,state.chordDisplay,key))}</span>${item.number?`<span>${escapeHtml(item.number)}</span>`:''}`;button.onclick=()=>applyChordChoice(song,storedChordToDisplay(canonical,song,state.chordDisplay,key));button.ondragstart=event=>{dragData={kind:'shelf-chord',value:canonical};event.dataTransfer.setData('text/plain',canonical);event.dataTransfer.effectAllowed='copy';};button.ondragend=()=>dragData=null;grid.append(button);});els.workbenchBody.append(grid);};
    addSection('Recent',(song.recentChords||[]).map(chord=>({chord})));
    addSection('In key',CE.keyScaleChords(key,song.spelling));
    addSection('Colour',CE.borrowedChords(key,song.spelling));
  ...
  function renderChart(song){const view=document.createElement('div');view.className='chart-view';const page=document.createElement('article');page.className='chart-page';page.style.fontSize=`${state.chartFontSize}px`;page.innerHTML=`<h1 class="chart-title">${escapeHtml(song.title)}</h1><div class="chart-meta">${escapeHtml(chartMeta(song))}</div>`;song.sections.forEach(section=>{const sectionEl=document.createElement('section');sectionEl.className='chart-section';sectionEl.innerHTML=`<h3>${escapeHtml(section.label)}${section.keyOverride?` · ${escapeHtml(section.keyOverride)}`:''}</h3>`;section.lines.forEach(line=>{if(state.chartMode==='progressions'&&line.kind!=='progression'&&!line.chords.length)return;if(state.chartMode==='lyrics'&&line.kind==='progression')return;const row=document.createElement('div');row.className=`chart-line${line.kind==='progression'?' chart-chord-row':''}`;if(line.style?.color&&line.style.color!=='ink')row.dataset.textColor=line.style.color;if(line.label&&line.kind==='progression'){const label=document.createElement('span');label.className='chart-row-label';label.textContent=line.label;row.append(label);}if(state.chartMode==='lyrics')row.append(document.createTextNode(line.text));else if(line.kind==='progression'){if(state.chartMode==='dual'){const shapes=document.createElement('div');shapes.textContent=displayProgression(line.text,song,'shapes',sectionKey(song,section));const sounding=document.createElement('div');sounding.className='secondary-chords';sounding.textContent=displayProgression(line.text,song,'sounding',sectionKey(song,section));row.append(shapes,sounding);}else row.append(document.createTextNode(displayProgression(line.text,song,state.chartMode==='progressions'?'sounding':state.chartMode,sectionKey(song,section))));}else if(state.chartMode==='progressions')row.append(document.createTextNode((line.chords||[]).map(chord=>storedChordToDisplay(chord.value,song,'sounding',sectionKey(song,section))).join('  ')));else if(state.chartMode==='dual'){renderStaticBracket(row,lineToBracket(song,section,line,'shapes'));const secondary=document.createElement('div');secondary.className='secondary-chords';secondary.textContent=(line.chords||[]).map(chord=>storedChordToDisplay(chord.value,song,'sounding',sectionKey(song,section))).join('  ');row.append(secondary);}else renderStaticBracket(row,lineToBracket(song,section,line,state.chartMode));sectionEl.append(row);});page.append(sectionEl);});view.append(page);els.viewHost.append(view);}

  function renderStaticBracket(container,text){const regex=/\[([^\]]+)\]/g;let last=0,match;while((match=regex.exec(text))){if(match.index>last){const span=document.createElement('span');span.textContent=text.slice(last,match.index);container.append(span);}const rest=text.slice(regex.lastIndex);const wordMatch=rest.match(/^(\s*)(\S+)?/);const spaces=wordMatch?.[1]||'';if(spaces){const span=document.createElement('span');span.textContent=spaces;container.append(span);}const group=document.createElement('span');group.className='anchor-group';const chord=document.createElement('span');chord.className='anchor-chord';chord.textContent=match[1];const word=document.createElement('span');word.className='anchor-word';word.textContent=wordMatch?.[2]||' ';group.append(chord,word);container.append(group);last=regex.lastIndex+spaces.length+(wordMatch?.[2]?.length||0);regex.lastIndex=last;}if(last<text.length){const span=document.createElement('span');span.textContent=text.slice(last);container.append(span);}}
  ...
  function openChartLayout(song){openFormModal('Chart layout',[{name:'size',label:'Chart font size',type:'select',value:String(state.chartFontSize),options:[12,14,16,18,20,22].map(value=>({value:String(value),label:`${value} pt`}))}],values=>{state.chartFontSize=Number(values.size);scheduleSave();renderView(song);});}
  function openChartExport(song){openChoiceModal('Export chart',[['Print / PDF',()=>window.print()],['Plain text',()=>downloadText(`${safeName(song.title)}.txt`,chartText(song))],['ChordPro',()=>downloadText(`${safeName(song.title)}.cho`,chordProText(song))],['Complete song bundle',()=>exportSongBundle(song)],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.5',song},null,2))]]);}
  ...
  function applyTransposePanel(song,panel,preserve){
    ...
    perform(`Transposed ${panel.scope}`,()=>{
      if(context.scope==='song'){
        ...
        song.key=targetKey;song.spelling=spelling;song.sections.forEach(section=>{if(section.keyOverride){const parsed=CE.parseKey(section.keyOverride);section.keyOverride=`${CE.noteName(CE.noteIndex(parsed.tonic)+semitones,targetKey,spelling)}${parsed.mode==='minor'?'m':''}`;}section.lines.forEach(line=>{if(line.kind==='progression')line.text=String(line.text||'').split(/(\s+|\|+)/g).map(part=>CE.parseChord(part)?.recognised?CE.transposeChord(part,semitones,targetKey,spelling):part).join('');else line.chords=(line.chords||[]).map(chord=>({...chord,value:CE.transposeChord(chord.value,semitones,targetKey,spelling)}));});});song.chartProfiles.forEach(profile=>{if(tuningIsAutomatic(profile.tuning))profile.capo=CE.capoForKeysWithTuning(targetKey,profile.shapeKey,profile.tuning);profile.name=`${profile.shapeKey} shapes · ${profile.capo?`capo ${profile.capo}`:'no capo'}`;});
  ...
  function renderKeyCapoPanel(song){
    ...
    const updateFromControls=(changed)=>{
      const nextTuning=tuning.value;if(nextTuning==='__custom__'){openCustomTuning(song,profile);return;}
      snapshot('Changed playing profile');const previousSound=song.key;const nextShape=shape.value;const nextCapo=Number(capo.value);const automatic=tuningIsAutomatic(nextTuning);
      if(!automatic){profile.tuning=nextTuning;profile.shapeKey=nextShape;profile.capo=nextCapo;song.tuning=nextTuning;}
      else if(profile.keepMode==='sounding'){
        profile.tuning=nextTuning;
        if(changed==='shape'){profile.shapeKey=nextShape;profile.capo=CE.capoForKeysWithTuning(song.key,profile.shapeKey,profile.tuning);}
        else if(changed==='capo'){profile.capo=nextCapo;profile.shapeKey=CE.shapeKeyForCapoWithTuning(song.key,profile.capo,profile.tuning,song.spelling);}
        else {profile.shapeKey=profile.shapeKey||nextShape;profile.capo=CE.capoForKeysWithTuning(song.key,profile.shapeKey,profile.tuning);}
      }else{
        profile.shapeKey=nextShape;profile.capo=nextCapo;profile.tuning=nextTuning;
      ...
      song.tuning=profile.tuning;profile.name=`${profile.shapeKey} shapes · ${profile.capo?`capo ${profile.capo}`:'no capo'}`;touch(song);render();toast(profile.keepMode==='sounding'?`Still sounds in ${compactKey(song.key)}. Now using ${profile.shapeKey} shapes${profile.capo?` with capo ${profile.capo}`:''}.`:`Now sounds in ${compactKey(song.key)}.`,undo);
    };
  ...
  function openSongMenu(anchor){
    const song=currentSong();if(!song)return;
    if(inSandboxMode()){openMenu(anchor,[['Return to demo song',returnToDemoSong],['Reset this sandbox song',()=>{if(confirm(`Reset “${song.title}” to its seed data?`))resetCurrentSandboxSong();}],['Reset sandbox library',()=>{if(confirm('Reset all eight sandbox songs?'))resetSandboxLibrary();}],['Save a copy',saveCurrentSandboxCopy],['Exit demo',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],['Export sandbox song JSON',()=>downloadText(`${safeName(song.title)}-sandbox.json`,JSON.stringify({type:'cowriter-song-0.4.0.5',song},null,2))]]);return;}
    if(inDemoMode()){openMenu(anchor,[[demoSession.guideOpen?'Hide demo guide':'Show demo guide',()=>{demoSession.guideOpen=!demoSession.guideOpen;if(demoSession.guideOpen){demoSession.guideStep=clamp(demoSession.guideStep||0,0,DEMO_STEPS.length-1);state.view=DEMO_STEPS[demoSession.guideStep].view;}scheduleSave();render();}],['Jump to demo stage…',()=>openChoiceModal('Jump to demo stage',[['Idea',()=>setDemoStage('idea')],['Draft',()=>setDemoStage('draft')],['Chart-ready',()=>setDemoStage('chart-ready')]])],['Open Sandbox Library',enterSandbox],['Reset demo',()=>{if(confirm('Reset every change made in this demo?'))resetDemo();}],['Save a copy',saveDemoCopy],['Back to library',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)]]);return;}
    openMenu(anchor,[['Export complete song…',()=>exportSongBundle(song)],['Studio Log',()=>openStudioLog(song)],['Record Work Tape',()=>startWorkTape(song)],['Change progress…',()=>openStagePicker(song,anchor)],['Add to project…',()=>openProjectMembership(song)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Key, capo and tuning…',()=>openKeyCapoPanel(song)],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],[song.favourite?'Remove favourite':'Favourite',()=>{song.favourite=!song.favourite;touch(song);render();}],['Duplicate song',()=>duplicateSong(song)],['Export song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.5',song},null,2))],['Archive',()=>{song.libraryStatus='archived';touch(song);render();}],['Move to Trash',()=>{snapshot('Move song to Trash');state.deletedSongs.unshift(song);state.songs=state.songs.filter(item=>item.id!==song.id);state.selectedSongId=state.songs[0]?.id||null;scheduleSave();render();toast('Song moved to Trash',undo);},'danger']]);
  }
  ...
  function openMenu(anchor,items){closeOverlay();const rect=anchor.getBoundingClientRect();const backdrop=document.createElement('div');backdrop.className='overlay-backdrop clear';const menu=document.createElement('div');menu.className='menu-popover';menu.style.left=`${Math.max(10,Math.min(window.innerWidth-235,rect.right-225))}px`;menu.style.top=`${Math.min(window.innerHeight-340,rect.bottom+6)}px`;items.forEach(([label,action,kind])=>{const button=document.createElement('button');button.textContent=label;if(kind==='danger')button.className='danger';button.onclick=()=>{closeOverlay();action();};menu.append(button);});backdrop.append(menu);backdrop.onclick=event=>{if(event.target===backdrop)closeOverlay();};els.overlayLayer.append(backdrop);}
  function openModal(title,body,footer=[]){closeOverlay();const backdrop=document.createElement('div');backdrop.className='overlay-backdrop';const modal=document.createElement('section');modal.className='modal';const head=document.createElement('header');head.className='modal-head';const heading=document.createElement('h3');heading.textContent=title;const close=document.createElement('button');close.className='icon-button';close.textContent='×';close.onclick=closeOverlay;head.append(heading,close);const content=document.createElement('div');content.className='modal-body';content.append(body);modal.append(head,content);if(footer.length){const foot=document.createElement('footer');foot.className='modal-foot';footer.forEach(item=>foot.append(item));modal.append(foot);}backdrop.append(modal);backdrop.onclick=event=>{if(event.target===backdrop)closeOverlay();};els.overlayLayer.append(backdrop);return{modal,content};}
  ...
  function renderDemoGuide(){
    ...
    const step=DEMO_STEPS[index];els.demoGuideProgress.textContent=`${index+1} of ${DEMO_STEPS.length}`;
    els.demoGuideBody.replaceChildren();
    const purpose=document.createElement('div');purpose.className='demo-guide-purpose';purpose.innerHTML='<strong>Plan</strong> — write freely<br><strong>Shape</strong> — move the pieces<br><strong>Write</strong> — build the song<br><strong>Chart</strong> — play and share';
    ...
    els.demoGuideBody.append(purpose,title,copy);
  ...
  function changeDemoGuideStep(direction){if(!inGuidedDemoMode())return;const next=clamp((demoSession.guideStep||0)+direction,0,DEMO_STEPS.length-1);if(direction>0&&demoSession.guideStep===DEMO_STEPS.length-1){demoSession.guideOpen=false;scheduleSave();render();toast('Demo guide complete');return;}demoSession.guideStep=next;state.view=DEMO_STEPS[next].view;demoSession.song.lastView=state.view;state.workbenchOpen=false;resetActiveSelection();scheduleSave();render();}

  function createNewSong(){if(inDemoMode())exitDemo(false);const song=createSong();state.songs.unshift(song);state.selectedSongId=song.id;state.view='plan';state.shellPage='workspace';state.sidebarCollapsed=true;state.workbenchOpen=false;state.alternativesTrayOpen=false;resetActiveSelection();scheduleSave();render();requestAnimationFrame(()=>document.querySelector('.brain-dump')?.focus());}
  ...
  function enterDemo(startGuide=false){
    ensureDemoSessionBase();demoSession.mode='demo';demoSession.song ||= demoSong('draft');demoSession.guideOpen=Boolean(startGuide);
  ...
  function loadDemo(){
    openChoiceModal('Explore Co-Writer Pro',[['Explore Demo Song',()=>enterDemo(false)],['Start seven-step guide',()=>enterDemo(true)],['Open Sandbox Library',enterSandbox]],'Explore the workflow in the demo song, or use the Sandbox Library for unrestricted testing.');
  }
  ...
  async function exportSongBundle(song){
    ...
    for(const take of song.takes){const blob=await audioGet(take.id);if(!blob){missing.push(take.name);continue;}const path=`audio/${take.id}.${audioExtension(take)}`;files[path]=new Uint8Array(await blob.arrayBuffer());audio.push({takeId:take.id,path,mimeType:take.mimeType,size:blob.size});}
    const manifest={type:'cowriter-song-bundle',version:'0.4.0.5',exchangeId,createdAt,contributor,sourceSongId:song.sourceSongId||song.id,songId:song.id,title:song.title,audio,missingAudio:missing};
    files['manifest.json']=JSON.stringify(manifest,null,2);files['song.json']=JSON.stringify(song,null,2);files['README.txt']=`Co-Writer Pro 0.4.0.5 Field Exchange\n\nSong: ${song.title}\nSent by: ${contributor}\nCreated: ${new Date(createdAt).toLocaleString()}\n\nImport this .cowriter-song.zip from Welcome or Library.\n${missing.length?`\nMissing audio: ${missing.join(', ')}\n`:''}`;
  ...
  function exportBackup(){downloadText(`Co-Writer-Pro-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({type:'cowriter-pro-backup',version:'0.4.0.5',exportedAt:now(),audioNotice:'Use Export complete song for a transferable bundle containing Work Tapes.',state},null,2));}
  ...
  function bind(){
    ...
    const openLibrary=()=>{state.filter='active';state.shellPage='library';state.sidebarCollapsed=false;scheduleSave();render();};
    ...
    els.homeButton.onclick=()=>{state.shellPage='welcome';state.sidebarCollapsed=true;scheduleSave();render();};
    els.libraryHome.onclick=()=>{state.shellPage='welcome';state.sidebarCollapsed=true;scheduleSave();render();};
    els.libraryMenu.onclick=()=>openMenu(els.libraryMenu,[['Explore demo',loadDemo],['Import song or backup…',chooseImport],['Backup library metadata',exportBackup],['Install for offline',installForOffline]]);
    els.loadDemo.onclick=loadDemo;
    els.closeDemoGuide.onclick=()=>{if(!inDemoMode())return;demoSession.guideOpen=false;scheduleSave();render();};els.demoGuideBack.onclick=()=>changeDemoGuideStep(-1);els.demoGuideNext.onclick=()=>changeDemoGuideStep(1);
    ...
    els.modeNav.onclick=event=>{const button=event.target.closest('button[data-view]');if(button)setView(button.dataset.view);};
    ...
    els.undoButton.onclick=undo;els.redoButton.onclick=redo;els.themeToggle.onclick=()=>{state.theme=state.theme==='light'?'dark':'light';scheduleSave();render();};els.songMenu.onclick=()=>openSongMenu(els.songMenu);
    ...
    window.addEventListener('appinstalled',()=>{deferredInstallPrompt=null;updateConnectionStatus();toast('Co-Writer Pro installed');});
    ...
    document.addEventListener('keydown',event=>{const mod=event.metaKey||event.ctrlKey,editing=['INPUT','TEXTAREA'].includes(document.activeElement?.tagName);if(event.code==='Space'&&!editing){spaceHeld=true;if(state.view==='shape')event.preventDefault();}if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='c'&&activeLineId){event.preventDefault();copyActiveChords(currentSong());}else if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='v'&&activeLineId){event.preventDefault();pasteActiveChords(currentSong());}else if(mod&&state.view==='write'&&!editing&&event.key.toLowerCase()==='d'&&activeLineId){event.preventDefault();duplicateActiveChords(currentSong());}else if(mod&&!event.shiftKey&&event.key.toLowerCase()==='z'){event.preventDefault();undo();}else if(mod&&event.shiftKey&&event.key.toLowerCase()==='z'){event.preventDefault();redo();}else if(mod&&event.key.toLowerCase()==='n'){event.preventDefault();createNewSong();}else if(mod&&event.key.toLowerCase()==='f'&&currentSong()){event.preventDefault();openSongSearch(currentSong());}else if(mod&&event.key.toLowerCase()==='k'&&state.view==='write'){event.preventDefault();addChordFromToolbar(currentSong());}else if(state.view==='shape'&&!mod&&!editing&&event.key.toLowerCase()==='v'){setShapeTool('select',currentSong());}else if(state.view==='shape'&&!mod&&!editing&&event.key.toLowerCase()==='h'){setShapeTool('hand',currentSong());}else if(event.key==='Escape'){closeSelectionBar();closeOverlay();if(state.alternativesTrayOpen){closeAlternativesTray();}else if(transientPanel){transientPanel=null;state.workbenchOpen=false;scheduleSave();render();}else if(activeEditor)activeEditor.blur();}});
  ...
  async function init(){bind();window.CoWriterMorningLines?.init();registerOfflineShell();db=await openDatabase();const loaded=await dbGet(STATE_KEY);const needsMigration=loaded&&Number(loaded.schemaVersion||0)<12;if(needsMigration)await dbSet('migration-backup-pre-0.4.0.5',loaded);state=normaliseState(loaded);if(!state.songs.length){state.songs=[];state.selectedSongId=null;state.shellPage='welcome';}else if(!state.songs.some(song=>song.id===state.selectedSongId))state.selectedSongId=state.songs[0].id;try{const savedDemo=JSON.parse(sessionStorage.getItem(DEMO_SESSION_KEY)||'null');if(savedDemo?.active&&(savedDemo.song||savedDemo.sandboxSongs)){demoSession=savedDemo;demoSession.mode ||= 'demo';if(savedDemo.song)demoSession.song=normaliseSong(savedDemo.song);if(Array.isArray(savedDemo.sandboxSongs))demoSession.sandboxSongs=savedDemo.sandboxSongs.map(normaliseSong);state.shellPage='workspace';}}catch(error){sessionStorage.removeItem(DEMO_SESSION_KEY);}if(currentSong())state.view=inSandboxMode()?currentSong().lastView||'write':inDemoMode()?(demoSession.guideOpen?DEMO_STEPS[demoSession.guideStep||0].view:demoSession.song.lastView||'write'):currentSong().lastView||state.view;render();window.CoWriterReviewCapture?.create({getState:()=>runtimeSnapshot(),getContext:()=>{const song=currentSong();return{page:document.body.classList.contains('morning-lines-open')?'morning-lines':state.shellPage,view:state.view,songId:song?.id||null,songTitle:song?.title||null,shelf:state.workbenchOpen?state.workbenchTab:null,alternativesOpen:state.alternativesTrayOpen};},notify:toast});}
  init().catch(error=>{console.error(error);document.body.innerHTML='<pre style="padding:30px">Co-Writer Pro could not start. Open the browser console for details.</pre>';});
})();
