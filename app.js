(() => {
  ...
  const DEMO_SESSION_KEY = `cowriter-pro-demo-0.4.0.18${RELEASE_CHANNEL === 'stable' ? '' : `-${RELEASE_CHANNEL}`}`;
  ...
  let shapeFrameValidationKey = null;
...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  const DEMO_STEPS = [
    {view:'plan',mode:'Plan',title:'Plan',copy:'Keep notes and rough lines here.',task:'Type or edit one sentence.',target:'.brain-dump'},
    {view:'write',mode:'Write',title:'Write',copy:'Edit lyrics with chords attached.',task:'Type in the blank Verse 2 row, then add a chord above a word.',target:'[data-demo-key="blank-demo-row"]'},
    {view:'shape',mode:'Shape',title:'Shape',copy:'Reorder sections and loose lines.',task:'Drag “The porch light stayed on…” to a new position.',target:'[data-demo-key="porch-light"]'},
    {view:'chart',mode:'Chart',title:'Chart',copy:'Format a clean copy to play or share.',task:'Change the chord display, then open Export.',target:'.chart-page'}
  ];
  ...
  async function startWorkTape(song=null,source='workspace',ideaId=null){
    ...
    try{
      ...
      updateRecordingTransport();recordingTimer=setInterval(updateRecordingTransport,250);toast('Recording work tape');
  ...
  async function openWorkTapeTrimmer(take,blob,target){
    ...
    const body=document.createElement('div');body.className='work-tape-trimmer';const intro=document.createElement('p');intro.className='trim-intro';intro.textContent='Drag either edge to choose what to keep.';
  ...
  async function turnIdeaIntoSong(idea){
    const song=createSong(ideaTitle(idea));state.songs.unshift(song);state.selectedSongId=song.id;await useIdeaInSong(idea,song,false);state.view='plan';state.shellPage='workspace';state.sidebarCollapsed=true;scheduleSave();render();toast('Song created. Idea kept in Ideas.');
  }
  ...
  function openStudioLog(song){
    ...
    const record=document.createElement('button');record.className='ghost-button studio-record-button';record.textContent='● Record work tape';record.onclick=()=>{closeOverlay();startWorkTape(song,'studio-log');};body.append(record);
    ...
    entries.forEach(entry=>{const row=document.createElement('article');row.className=`studio-log-entry ${entry.type}`;const take=entry.type==='take'?song.takes.find(item=>item.id===entry.takeId):null;const head=document.createElement('header');const label=document.createElement('div');const entryLabel=take?.name||(entry.type==='note'?'Song note':entry.type==='exchange'?'Imported song':'Missing work tape');label.innerHTML=`<strong>${escapeHtml(entryLabel)}</strong><small>${escapeHtml(new Date(entry.createdAt).toLocaleString())}${entry.view?` · ${escapeHtml(entry.view)}`:''}${take?` · ${escapeHtml(formatDuration(take.durationMs))}`:''}</small>`;head.append(label);row.append(head);
      ...
      list.append(row);});body.append(list);openModal('Recordings & notes',body);
  }
  ...
  function render() {
    ...
    els.songContext.innerHTML = `${inSandboxMode()?'<span class="demo-badge sandbox-badge">PRACTICE · CHANGES STAY HERE</span>':inDemoMode()?'<span class="demo-badge">DEMO · CHANGES STAY HERE</span>':''}`;
    renderToolbar(song);
    renderView(song);
    renderWorkbench();
  ...
  function renderLibrary() {
    ...
    els.projectsPanel.classList.toggle('hidden',inDemoMode()||!state.projects.length);
    ...
    if(!songs.length){
      ...
      empty.innerHTML=`<h2>${escapeHtml(copy)}</h2>`;
  ...
  function syncViewContext(song,fromView,toView){
    if(toView==='shape'&&fromView==='write'){
      ...
      if(found||section){shapeSelection.clear();shapeSelection.add(found?shapeKey('line',found.line.id):shapeKey('section',section.id));activeSectionId=section.id;rememberShapeSelection(song);}
    }
  ...
  function setView(view) {
    ...
    requestAnimationFrame(()=>{const memory=song.viewMemory?.[view];if(memory?.scrollTop!==undefined)els.viewHost.scrollTop=memory.scrollTop;if(view==='plan'&&!song.plan.brainDump)document.querySelector('.brain-dump')?.focus();if(view==='shape'&&previousView!=='shape')fitShapeViewIfNeeded(song);revealViewContext(song,view);});
  }
  ...
  function renderToolbar(song) {
    ...
    if(state.view!=='chart'&&!inDemoMode())right.prepend(toolButton('● Record',mediaRecorder?'A work tape is recording':'Record a work tape or open recordings and notes',event=>mediaRecorder?toast('Use the recording controls below'):openChoicePopover(event.currentTarget,'Recordings & notes',[['Record work tape',()=>startWorkTape(song)],['Open recordings & notes',()=>openStudioLog(song)]]),song.takes.length?'has-takes':''));
    els.utilityToolbar.append(left,right);
  }

  function openContextualTools(song,anchor,context){
    const items=[];
    if(context==='plan')items.push(['From Ideas',()=>chooseIdeaForPlan(song)],['Save selection as idea',()=>savePlanSelectionAsIdea(song)],['Add selection to Shape',()=>sendPlanToShape(song)],['Text appearance',()=>openTextMenu(song,anchor,'plan')],['Player setup',()=>openInlineHarmonyPopover(song,anchor)]);
    ...
    if(!inDemoMode())items.push(['Record work tape',()=>startWorkTape(song)],['Open recordings & notes',()=>openStudioLog(song)]);
    openChoicePopover(anchor,'More tools',items);
  }
  function renderPhoneToolbar(song,left,right){
    if(state.view==='plan')left.append(toolButton('Add to Write','Copy selected writing into the lyric notebook',()=>sendPlanToWrite(song),'primary-action'));
    ...
    if(state.view!=='shape'||!inDemoMode()){const more=toolButton('More','Open contextual tools',event=>openContextualTools(song,event.currentTarget,state.view));right.append(more);}
  }
  ...
  function renderMobileShape(song){
    if(!song.sections.some(section=>section.id===mobileShapeExpandedSectionId))mobileShapeExpandedSectionId=null;
    const view=document.createElement('div');view.className='shape-mobile';const intro=document.createElement('header');intro.className='shape-mobile-intro';intro.innerHTML='<h2>Reorder sections and loose lines</h2>';view.append(intro);
    const loose=document.createElement('details');loose.className='shape-mobile-group loose';loose.open=song.shapeBlocks.length<=2;const looseHead=document.createElement('summary');looseHead.innerHTML=`<div><strong>Loose lines</strong><small>${song.shapeBlocks.length} ${song.shapeBlocks.length===1?'piece':'pieces'}</small></div><span>${loose.open?'Hide':'Open'}</span>`;loose.ontoggle=()=>{const label=looseHead.querySelector(':scope > span');if(label)label.textContent=loose.open?'Hide':'Open';};const addLoose=document.createElement('button');addLoose.className='text-button shape-mobile-loose-add';addLoose.textContent='+ Add loose line';addLoose.onclick=()=>addMobileShapeBlock(song);loose.append(looseHead,addLoose);
    const orderedBlocks=song.shapeBlocks.slice().sort((a,b)=>(a.y-b.y)||(a.x-b.x));
  ...
  function renderShape(song) {
    ...
    view.append(viewport,zoom);els.viewHost.append(view);const frameKey=`${song.id}:${viewport.clientWidth}x${viewport.clientHeight}`;if(shapeFrameValidationKey!==frameKey){shapeFrameValidationKey=frameKey;requestAnimationFrame(()=>fitShapeViewIfNeeded(song));}
  }
  ...
  function fitShapeViewIfNeeded(song){
    if(phoneShapeLayout())return;
    const viewport=document.querySelector('.shape-viewport'),bounds=shapeBounds(song,false);if(!viewport||!bounds)return;
    const zoom=song.shapeView.zoom||1,left=song.shapeView.panX+bounds.x*zoom,top=song.shapeView.panY+bounds.y*zoom,right=song.shapeView.panX+bounds.right*zoom,bottom=song.shapeView.panY+bounds.bottom*zoom,margin=28;
    const outside=left<margin||top<margin||right>viewport.clientWidth-margin||bottom>viewport.clientHeight-margin;
    const contentWidth=Math.max(1,right-left),contentHeight=Math.max(1,bottom-top),tooSmall=contentWidth<viewport.clientWidth*.36&&contentHeight<viewport.clientHeight*.32;
    if(outside||tooSmall)fitShapeView(song,false);
  }
  ...
  function openShapeBlockMenu(song,block,anchor){
    ...
    openMenu(anchor,[['Edit loose line',()=>{shapeEditingKey=shapeKey('block',block.id);shapeSelection.clear();shapeSelection.add(shapeEditingKey);rememberShapeSelection(song);renderView(song);}],['Move to Write',()=>{shapeSelection.clear();shapeSelection.add(shapeKey('block',block.id));sendShapeSelectionToWrite(song);}],...(destinations.length?[['Move to section…',()=>openChoiceModal('Move to section',destinations)]]:[]),['Convert to loose line',()=>{block.type='fragment';touch(song);render();}],['Convert to title',()=>{block.type='title';touch(song);render();}],['Convert to progression',()=>{block.type='harmony';touch(song);render();}],['Delete',()=>perform('Deleted loose line',()=>song.shapeBlocks=song.shapeBlocks.filter(item=>item.id!==block.id),'Loose line deleted'),'danger']]);
  }
  ...
  function sendShapeSelectionToWrite(song){
    ...
    perform('Move Shape material to Write',()=>{
    ...
    },`Moved to ${target?.label||'Verse 1'}`,()=>setView('write'));
  }
  ...
  function renderAlternativesTray(){
    ...
    const currentValue=activeChordId?findChord(song,activeChordLineId,activeChordId)?.chord?.value||'':found?.line?(found.line.text||'New blank version'):findSection(song,activeSectionId)?.label||'Current choice';
    const current=document.createElement('article');current.className='shelf-card alternative-choice current';current.innerHTML=`<div class="alternative-letter">A</div><div class="shelf-meta">Current</div><div class="shelf-copy">${escapeHtml(currentValue)}</div>`;els.alternativesTrayBody.append(current);
  ...
  function openLineMenu(song,section,line,anchor){
    const copyToShape=()=>perform('Copied line to loose lines',()=>song.shapeBlocks.push(createBlock(line.kind==='progression'?'harmony':'fragment',line.text,section.shape.x+section.shape.w+40,section.shape.y+50,line.chords)),'Copied to loose lines');
    const move=[['Move up',()=>moveLine(song,section.id,line.id,section.id,Math.max(0,section.lines.indexOf(line)-1))],['Move down',()=>moveLine(song,section.id,line.id,section.id,Math.min(section.lines.length-1,section.lines.indexOf(line)+1))],['Move to section…',()=>openMoveLineMenu(song,section,line,anchor)],['Make loose',()=>moveLineToSpare(song,section,line)]];
    const lineActions=[['Copy to loose lines',copyToShape],['Duplicate',()=>perform('Duplicated line',()=>{const index=section.lines.indexOf(line);const copy=clone(line);copy.id=uid('line');copy.chords.forEach(chord=>chord.id=uid('chord'));section.lines.splice(index+1,0,copy);},'Line duplicated')]];
    const chordActions=line.kind==='progression'?
      [['Copy chord row',()=>{activeLineId=line.id;copyActiveChords(song);}],['Paste chord row',()=>{activeLineId=line.id;pasteActiveChords(song);}],['Duplicate chord row',()=>{activeLineId=line.id;copyActiveChords(song);pasteActiveChords(song);}],['Add bar',()=>perform('Added bar',()=>addProgressionBar(line),'Bar added')],['Transpose chord row…',()=>openTransposePanel(song,'progression',{lineId:line.id})],['Label chord row…',()=>openFormModal('Label chord row',[{name:'label',label:'Label',type:'text',value:line.label||'',placeholder:'Turnaround'}],values=>{line.label=values.label.trim();touch(song);render();})]]:
      [['Copy chords',()=>{activeLineId=line.id;activeChordId=null;copyActiveChords(song);}],['Paste chords',()=>{activeLineId=line.id;pasteActiveChords(song);}],['Duplicate chords',()=>{activeLineId=line.id;activeChordId=null;duplicateActiveChords(song);}],['Split line at cursor',()=>{const editor=document.querySelector(`.notebook-row[data-line-id="${CSS.escape(line.id)}"] .line-editor`);if(editor){const [before,after]=NE.splitLine(line,editor.selectionStart,()=>uid('line'));Object.assign(line,before);section.lines.splice(section.lines.indexOf(line)+1,0,after);touch(song);render();}}],['Merge with line above',()=>{const index=section.lines.indexOf(line);if(index>0){const previous=section.lines[index-1];perform('Merged lyric rows',()=>{Object.assign(previous,NE.mergeLines(previous,line));section.lines.splice(index,1);},'Lines merged');}}]];
    const store=['Store as alternative',()=>{song.alternatives.unshift({id:uid('alt'),type:line.kind==='progression'?'harmony':'lyric',targetType:'line',targetId:line.id,label:'Stored alternative',content:{line:clone({text:line.text,chords:line.chords,kind:line.kind,style:line.style})},createdAt:now()});touch(song);renderWorkbench();toast('Stored as an alternative');}];
    const remove=['Delete',()=>perform('Deleted line',()=>{section.lines=section.lines.filter(item=>item.id!==line.id);song.alternatives.forEach(alt=>{if(alt.targetType==='line'&&alt.targetId===line.id)alt.targetId=`unplaced:${line.id}`;});},'Line deleted'),'danger'];
    openMenu(anchor,[['Move…',()=>openChoicePopover(anchor,'Move row',move)],['Chords…',()=>openChoiceModal(line.kind==='progression'?'Chord row':'Chords',chordActions)],['Line…',()=>openChoiceModal('Line',lineActions)],[store[0],store[1]],[remove[0],remove[1],remove[2]]]);
  }
  ...
  function openChartLayout(song){
    const layout=song.chartLayout;const form=document.createElement('form');form.className='chart-layout-form';const fields={};
    const addGroup=(title,items)=>{const group=document.createElement('fieldset');const legend=document.createElement('legend');legend.textContent=title;group.append(legend);items.forEach(([name,label,value,options])=>{const wrap=document.createElement('label');wrap.className='field';wrap.textContent=label;const select=document.createElement('select');select.name=name;options.forEach(([optionValue,optionLabel])=>{const option=document.createElement('option');option.value=String(optionValue);option.textContent=optionLabel;select.append(option);});select.value=String(value);fields[name]=select;wrap.append(select);group.append(wrap);});form.append(group);};
    addGroup('Typography',[['size','Text size',state.chartFontSize,[12,14,16,18,20,22].map(value=>[value,`${value} pt`])],['fontFamily','Chart typeface',layout.fontFamily,[['song','Match Write'],['sans','Contemporary'],['serif','Traditional'],['typewriter','Typewriter']]]]);
    addGroup('Page',[['pageSize','Page size',layout.pageSize,[['a4','A4'],['letter','US Letter']]],['orientation','Orientation',layout.orientation,[['portrait','Portrait'],['landscape','Landscape']]],['margin','Margins',layout.margin,[['compact','Compact'],['normal','Normal'],['wide','Wide']]],['columns','Columns',layout.columns,[['1','One column'],['2','Two columns']]]]);
  ...
  function openChartExport(song){openChoiceModal('Export chart',[['Print / PDF',()=>window.print()],['Plain text',()=>downloadText(`${safeName(song.title)}.txt`,chartText(song))],['Complete song bundle',()=>exportSongBundle(song)],['More export formats…',()=>openChoiceModal('More export formats',[['ChordPro',()=>downloadText(`${safeName(song.title)}.cho`,chordProText(song))],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.18',song},null,2))]])]]);}
  ...
  function openSongMenu(anchor){
    ...
    if(inSandboxMode()){openMenu(anchor,[['Back to demo song',returnToDemoSong],['Reset this practice song',()=>{if(confirm(`Reset “${song.title}” to its starting version?`))resetCurrentSandboxSong();}],['Reset all practice songs',()=>{if(confirm('Reset all eight practice songs?'))resetSandboxLibrary();}],['Save copy to Songs',saveCurrentSandboxCopy],['Exit practice',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],['Export song data (JSON)',()=>downloadText(`${safeName(song.title)}-sandbox.json`,JSON.stringify({type:'cowriter-song-0.4.0.18',song},null,2))]]);return;}
    if(inDemoMode()){
      ...
      const demoActions=[['Open practice songs',enterSandbox],['Reset demo song',()=>{if(confirm('Reset every change made in this demo?'))resetDemo();}],['Save copy to Songs',saveDemoCopy],['Exit demo',leaveDemoForWelcome]];
      openChoicePopover(anchor,'Demo',[['Guided tour',()=>openChoiceModal('Guided tour',tour)],['Song tools',()=>openChoiceModal('Song tools',songTools)],['Demo options',()=>openChoiceModal('Demo options',demoActions)]]);return;
    }
    ...
    const exports=[['Export complete song…',()=>exportSongBundle(song)],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.18',song},null,2))]];
  ...
  function renderDemoGuide(){
    ...
    const step=DEMO_STEPS[index];els.demoGuide.querySelector('.demo-guide-kicker').textContent=demoSession.unattended?'AUTOMATIC TOUR':'GUIDED TOUR';els.demoGuideProgress.textContent=`${index+1} of ${DEMO_STEPS.length} · ${step.mode}`;
    els.demoGuideBody.replaceChildren();
    const purpose=document.createElement('div');purpose.className='demo-guide-purpose demo-guide-safety';purpose.innerHTML='<strong>Demo</strong><span>Changes stay out of Songs.</span>';
  ...
  function pauseDemoGuide(){if(!inGuidedDemoMode())return;stopUnattendedDemo();demoSession.unattended=false;demoSession.guideOpen=false;scheduleSave();render();toast('Tour paused · reopen it from the song menu');}
  ...
  function enterSandbox(){
    ...
    if(!demoSession.sandboxSongs.length){toast('Practice songs could not be loaded');return;}
    ...
    const song=currentSong();state.filter='active';state.view=song?.lastView||'write';state.shellPage='workspace';state.sidebarCollapsed=true;state.workbenchOpen=false;resetActiveSelection();undoStack=[];redoStack=[];scheduleSave();render();toast('Practice songs opened');
  }
  ...
  function loadDemo(){
    openChoiceModal('Explore Co-Writer',[['Open demo song',()=>enterDemo(false)],['Start guided tour',()=>enterDemo(true)],['Play automatic tour',startUnattendedDemo],['Open practice songs',enterSandbox]],'Demo and practice songs are separate from Songs.');
  }
  ...
  async function exportSongBundle(song){
    ...
    const manifest={type:'cowriter-song-bundle',version:'0.4.0.18',exchangeId,createdAt,contributor,sourceSongId:song.sourceSongId||song.id,songId:song.id,title:song.title,audio,missingAudio:missing};
    files['manifest.json']=JSON.stringify(manifest,null,2);files['song.json']=JSON.stringify(song,null,2);files['README.txt']=`Co-Writer 0.4.0.18 Field Exchange\n\nSong: ${song.title}\nSent by: ${contributor}\nCreated: ${new Date(createdAt).toLocaleString()}\n\nImport this .cowriter-song.zip from Home or Songs.\n${missing.length?`\nMissing audio: ${missing.join(', ')}\n`:''}`;
  ...
  function exportBackup(){downloadText(`Co-Writer-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({type:'cowriter-pro-backup',version:'0.4.0.18',exportedAt:now(),audioNotice:'Use Export complete song for a transferable bundle containing work tapes.',state},null,2));}
  ...
  function bind(){
    ...
    els.welcomeCapture.onsubmit=event=>{event.preventDefault();saveHomeIdea();};els.welcomeIdeaInput.oninput=()=>{els.welcomeIdeaSave.disabled=!els.welcomeIdeaInput.value.trim();els.welcomeIdeaStatus.textContent='';autoGrow(els.welcomeIdeaInput,48);};els.welcomeIdeaInput.onkeydown=event=>{if((event.metaKey||event.ctrlKey)&&event.key==='Enter'){event.preventDefault();saveHomeIdea();}};
