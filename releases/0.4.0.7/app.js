(() => {
  ...
  const DEMO_SESSION_KEY = 'cowriter-pro-demo-0.4.0.7';
  ...
  const els = Object.fromEntries([
    'app','welcomePage','welcomeContinueCard','welcomeContinueTitle','welcomeContinueMeta','welcomeContinue','welcomeNewSong','welcomeLibrary','welcomeGetStarted','welcomeMainActions','welcomeQuietActions','welcomeMorningLines','welcomeRecordIdea','welcomeMore','welcomeInstall','welcomeConnection','welcomeDemo','welcomeImport','sidebar','libraryHome','libraryMenu','collapseSidebar','expandSidebar','homeButton','mainShell','newSong','emptyNewSong','emptyOpenLibrary','songSearch','songList','libraryFilters','libraryTools','libraryFilterButton','librarySortButton','projectsPanel','projectList','newProject','loadDemo','importBackup','exportBackup','backupFile',
...
G – D/F# – Em7 – Cadd9. Keep the verse conversational. Let the bridge lift somewhere brighter and make that brightness hurt.`;
  ...
  const DEMO_STEPS = [
    ...
    {view:'shape',mode:'Shape',title:'Move the pieces',copy:'Shape is the desk. Fragments can sit anywhere while you look for the form.',task:'Drag “The porch light stayed on…” to a place that feels right.',target:'[data-demo-key="porch-light"]'},
  ...
  async function finishWorkTape(){
    ...
    else{state.unattachedTakes.unshift(take);scheduleSave();renderWelcome();}
  ...
  function revealSavedWorkTape(take,target){
    ...
    else{renderWelcome();toast('Idea saved — choose where it belongs');openAttachCapture(take);}
  }
  ...
  function renderWelcome(){
    ...
    els.welcomeNewSong.classList.toggle('primary-button',!song);els.welcomeNewSong.classList.toggle('ghost-button',Boolean(song));
  ...
  function renderToolbar(song) {
    ...
    if (state.view==='plan') {
      ...
      right.append(toolButton('Arrange','Lay pieces out without overlaps',()=>arrangeShape(song)),toolButton(playerSetupLabel(song),'Sounding key, playing shapes and capo',event=>openInlineHarmonyPopover(song,event.currentTarget),'music-setup'),toolButton('Shelf','Open words or music',()=>toggleWorkbench(state.workbenchTab||'words'),state.workbenchOpen?'active':''));
  ...
  function arrangeShape(song){perform('Arranged Shape desk',()=>{const items=[...(song.sections||[]).map(section=>({kind:'section',item:section,w:section.shape.w||340,h:section.shape.h||220,x:section.shape.x,y:section.shape.y})),...(song.shapeBlocks||[]).map(block=>({kind:'block',item:block,w:block.w||220,h:block.h||100,x:block.x,y:block.y}))].sort((a,b)=>(a.y-b.y)||(a.x-b.x));let x=120,y=110,columnWidth=0;items.forEach(entry=>{if(y+entry.h>1050&&y>110){x+=columnWidth+70;y=110;columnWidth=0;}if(entry.kind==='section'){entry.item.shape.x=x;entry.item.shape.y=y;}else{entry.item.x=x;entry.item.y=y;}y+=entry.h+46;columnWidth=Math.max(columnWidth,entry.w);});},'Shape desk arranged');}
  ...
  function openChartExport(song){openChoiceModal('Export chart',[['Print / PDF',()=>window.print()],['Plain text',()=>downloadText(`${safeName(song.title)}.txt`,chartText(song))],['ChordPro',()=>downloadText(`${safeName(song.title)}.cho`,chordProText(song))],['Complete song bundle',()=>exportSongBundle(song)],['Song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.7',song},null,2))]]);}
  ...
  function openSongMenu(anchor){
    const song=currentSong();if(!song)return;
    if(inSandboxMode()){openMenu(anchor,[['Return to demo song',returnToDemoSong],['Reset this sandbox song',()=>{if(confirm(`Reset “${song.title}” to its seed data?`))resetCurrentSandboxSong();}],['Reset sandbox library',()=>{if(confirm('Reset all eight sandbox songs?'))resetSandboxLibrary();}],['Save a copy',saveCurrentSandboxCopy],['Exit demo',()=>exitDemo(true)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],['Export sandbox song JSON',()=>downloadText(`${safeName(song.title)}-sandbox.json`,JSON.stringify({type:'cowriter-song-0.4.0.7',song},null,2))]]);return;}
    ...
    openMenu(anchor,[['Export complete song…',()=>exportSongBundle(song)],['Studio Log',()=>openStudioLog(song)],['Record Work Tape',()=>startWorkTape(song)],['Change progress…',()=>openStagePicker(song,anchor)],['Add to project…',()=>openProjectMembership(song)],['Transpose song…',()=>openTransposePanel(song,'song')],['Set song key without moving chords…',()=>openInterpretPanel(song,'song')],['Key, capo and tuning…',()=>openKeyCapoPanel(song)],['Writers & IPI',()=>openWriters(song)],['Versions',()=>openVersions(song)],[song.favourite?'Remove favourite':'Favourite',()=>{song.favourite=!song.favourite;touch(song);render();}],['Duplicate song',()=>duplicateSong(song)],['Export song metadata JSON',()=>downloadText(`${safeName(song.title)}.json`,JSON.stringify({type:'cowriter-song-0.4.0.7',song},null,2))],['Archive',()=>{song.libraryStatus='archived';touch(song);render();}],['Move to Trash',()=>{snapshot('Move song to Trash');state.deletedSongs.unshift(song);state.songs=state.songs.filter(item=>item.id!==song.id);state.selectedSongId=state.songs[0]?.id||null;scheduleSave();render();toast('Song moved to Trash',undo);},'danger']]);
  }
  ...
  function openAbout(){const body=document.createElement('div');body.className='about-card';body.innerHTML='<strong>Co-Writer</strong><span>Private Alpha · 0.4.0.7</span><p>A songwriter’s notebook for planning, shaping, writing and sharing a song.</p><small>Your Library and recordings are saved on this device.</small>';openModal('About',body);}
  function openStartHere(){const body=document.createElement('div');body.className='start-here';body.innerHTML='<p class="start-here-intro">A song can begin anywhere. Choose the space that matches the work in front of you.</p><div class="start-here-path"><article><span>01</span><div><strong>Plan</strong><p>Think freely and keep the rough edges.</p></div></article><article><span>02</span><div><strong>Shape</strong><p>Move fragments until the form appears.</p></div></article><article><span>03</span><div><strong>Write</strong><p>Bring words and chords together.</p></div></article><article><span>04</span><div><strong>Chart</strong><p>Make a clean copy to play or share.</p></div></article></div>';const tour=document.createElement('button');tour.className='ghost-button';tour.textContent='Walk through a song';tour.onclick=()=>{closeOverlay();enterDemo(true);};const begin=document.createElement('button');begin.className='primary-button';begin.textContent='Start a blank song';begin.onclick=()=>{closeOverlay();createNewSong();};openModal('Start here',body,[tour,begin]);}
  ...
  async function exportSongBundle(song){
    ...
    const manifest={type:'cowriter-song-bundle',version:'0.4.0.7',exchangeId,createdAt,contributor,sourceSongId:song.sourceSongId||song.id,songId:song.id,title:song.title,audio,missingAudio:missing};
    files['manifest.json']=JSON.stringify(manifest,null,2);files['song.json']=JSON.stringify(song,null,2);files['README.txt']=`Co-Writer 0.4.0.7 Field Exchange\n\nSong: ${song.title}\nSent by: ${contributor}\nCreated: ${new Date(createdAt).toLocaleString()}\n\nImport this .cowriter-song.zip from Welcome or Library.\n${missing.length?`\nMissing audio: ${missing.join(', ')}\n`:''}`;
  ...
  function exportBackup(){downloadText(`Co-Writer-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({type:'cowriter-pro-backup',version:'0.4.0.7',exportedAt:now(),audioNotice:'Use Export complete song for a transferable bundle containing Work Tapes.',state},null,2));}
